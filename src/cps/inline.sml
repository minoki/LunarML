...
fun simplifySimpleExp (_ : env, C.Record _) = NOT_SIMPLIFIED
  | simplifySimpleExp (env, C.PrimOp { primOp, tyargs = _, args })
    = (case (primOp, args) of
         ...
         | (F.PrimCall (P.Int_TIMES_wrapping w), [C.IntConst (w', x'), C.IntConst (w'', y')]) =>
           if w = w' andalso w = w'' then
               VALUE (C.IntConst (w, wraparound (w, x' * y')))
           else
               NOT_SIMPLIFIED
...
and simplifyDec (ctx : Context) (dec, (env, cenv, subst, csubst, acc : C.Dec list))
    = case dec of
        ...
        | C.RecDec defs =>
          ...
          in (env, cenv, subst, csubst, C.RecDec defs :: acc)
          end
        | C.ContDec { name, params, body } =>
          let val body = simplifyCExp (ctx, env, cenv, subst, csubst, body)
              val cenv = C.CVarMap.insert (cenv, name, (params, if CpsSimplify.sizeOfCExp (body, 3) >= 0 then (* Inline very small continuations *)
                                                                    SOME body
                                                                else
                                                                    NONE
                                                       )
        ...
        | C.RecContDec defs =>
          let val dec = C.RecContDec (List.map (fn (name, params, body) => (name, params, simplifyCExp (ctx, env, cenv, subst, csubst, body))) defs)
          in (env, cenv, subst, csubst, dec :: acc)
          end
        | C.ESImportDec _ =>
        ...
        | C.App { applied, cont, args } =>
          ...
          in case applied of
                 ...
                 (case TypedSyntax.VIdMap.find (env, applied) of
                      ...
                      in CpsSimplify.alphaConvert (ctx, subst, csubst, body)
                      end
                          (*
...
*)
        ...
        | C.AppCont { applied, args } =>
          ...
          in case C.CVarMap.find (cenv, applied) of
                 ...
                 let val () = #simplificationOccurred ctx := true
                     val subst = ListPair.foldlEq (fn (SOME p, a, subst) => TypedSyntax.VIdMap.insert (subst, p, a)
                                                  | (NONE, _, subst) => subst
                                                  ) subst (params, args)
                 in CpsSimplify.alphaConvert (ctx, subst, csubst, body)
                 end
        ...
        | C.Unreachable => e
fun goCExp (ctx : CpsSimplify.Context, exp) = simplifyCExp (ctx, TypedSyntax.VIdMap.empty, C.CVarMap.empty, TypedSyntax.VIdMap.empty, C.CVarMap.empty, exp)
end (* local *)
