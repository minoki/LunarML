 ...
 *)
structure CpsDecomposeRecursive : sig
...
fun goDec ctx (dec, acc)
    = case dec of
        ...
        | C.ContDec { name, params, body } => C.ContDec { name = name, params = params, body = goCExp (ctx, body) } :: acc
        | C.RecContDec defs =>
          let val defs = List.map (fn (name, params, body) => (name, params, goCExp (ctx, body))) defs
          in C.RecContDec defs :: acc
          end
        | C.ESImportDec _ => dec :: acc
and goCExp (ctx : CpsSimplify.Context, exp)
    = case exp of
          C.Let { decs, cont } => C.Let { decs = List.rev (List.foldl (goDec ctx) [] decs), cont = goCExp (ctx, cont) }
        | C.App _ => exp
        | C.AppCont _ => exp
