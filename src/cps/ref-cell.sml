...
local
    ...
    val neverUsed : usage = { ref_read = NEVER
                            , ref_write = NEVER
                            , other = NEVER
                            }
    ...
    fun getValueUsage (table : usage_table, v)
        = case TypedSyntax.VIdTable.find table v of
              SOME r => !r
            | NONE => { ref_read = MANY, ref_write = MANY, other = MANY } (* unknown *)
    fun useValue env (C.Var v) = (case TypedSyntax.VIdTable.find env v of
                                      SOME r => let val { ref_read, ref_write, other } = !r
                                                in r := { ref_read = ref_read, ref_write = ref_write, other = oneMore other }
                                                end
                                    | NONE => ()
                                 )
      | useValue _ C.Unit = ()
    ...
    fun useValueAsRefRead (env, C.Var v)
        = (case TypedSyntax.VIdTable.find env v of
               SOME r => let val { ref_read, ref_write, other } = !r
                         in r := { ref_read = oneMore ref_read, ref_write = ref_write, other = other }
                         end
             | NONE => ()
          )
    ...
    fun useValueAsRefWrite (env, C.Var v)
        = (case TypedSyntax.VIdTable.find env v of
               SOME r => let val { ref_read, ref_write, other } = !r
                         in r := { ref_read = ref_read, ref_write = oneMore ref_write, other = other }
                         end
             | NONE => ()
          )
    ...
    in
    fun goSimpleExp (env, _, C.PrimOp { primOp = FSyntax.PrimCall Primitives.Ref_set, tyargs = _, args = [r, v] }) = (useValueAsRefWrite (env, r); useValue env v)
      | goSimpleExp (env, _, C.PrimOp { primOp = FSyntax.PrimCall Primitives.Ref_read, tyargs = _, args = [r] }) = useValueAsRefRead (env, r)
      | goSimpleExp (env, _, C.PrimOp { primOp = _, tyargs = _, args }) = List.app (useValue env) args
      | goSimpleExp (env, _, C.Record fields) = Syntax.LabelMap.app (useValue env) fields
      | goSimpleExp (_, _, C.ExnTag { name = _, payloadTy = _ }) = ()
      | goSimpleExp (env, _, C.Projection { label = _, record, fieldTypes = _ }) = useValue env record
    ...
    and goDec (env, renv)
      ...
      | C.RecDec defs =>
        ...
        in TypedSyntax.VIdMap.appi (fn (f, v) => TypedSyntax.VIdTable.insert env (f, v)) recursiveEnv
         ; List.app (fn { contParam = _, params, body, ... } =>
                        ( List.app (fn p => add (env, p)) params
                        ; goCExp (env, renv, body)
                        )
                    ) defs
    ...
    and goCExp (env : (usage ref) TypedSyntax.VIdTable.hash_table, renv, cexp)
        = case cexp of
              C.Let { decs, cont } =>
            ...
            | C.App { applied, cont = _, args, attr = _ } =>
              ( useValue env applied
              ; List.app (useValue env) args
              )
            | C.AppCont { applied = _, args } =>
              List.app (useValue env) args
...
and simplifyDec (ctx : Context) (dec, (env, subst, acc : C.Dec list))
    = case dec of
          ...
          let val exp = CpsSimplify.substSimpleExp (subst, C.CVarMap.empty, exp)
          in case simplifySimpleExp (#usage ctx, env, exp) of
               ...
               | simplified =>
                 ...
                 in case (exp, results) of
                        ...
                        let val body = simplifyCExp (ctx, env, subst, body)
                            val exp = C.Abs { contParam = contParam, params = params, body = body, attr = attr }
                            val env = TypedSyntax.VIdMap.insert (env, result, { exp = NONE })
                            val dec = C.ValDec { exp = exp
                                               , results = [SOME result]
                                               }
                        in (env, subst, dec :: acc)
                        end
                      | _ => (case (C.isDiscardable exp, results) of
                                ...
                                | (_, [SOME result]) => let val dec = C.ValDec { exp = exp
                                                            ...
                                                            val env = TypedSyntax.VIdMap.insert (env, result, { exp = SOME exp })
                                                        in (env, subst, dec :: acc)
                                                        end
        ...
        | C.RecDec defs =>
          let val defs = List.map (fn { name, contParam, params, body, attr } => { name = name, contParam = contParam, params = params, body = simplifyCExp (ctx, env, subst, body), attr = attr }) defs
        ...
        | C.RecContDec defs =>
          let val dec = C.RecContDec (List.map (fn (name, params, body) => (name, params, simplifyCExp (ctx, env, subst, body))) defs)
          in (env, subst, dec :: acc)
          end
        | C.ESImportDec _ => (env, subst, dec :: acc)
and simplifyCExp (ctx : Context, env : { exp : CSyntax.SimpleExp option } TypedSyntax.VIdMap.map, subst : C.Value TypedSyntax.VIdMap.map, e)
    = case e of
          C.Let { decs, cont } =>
          let val (env, subst, revDecs) = List.foldl (simplifyDec ctx) (env, subst, []) decs
          in CpsTransform.prependRevDecs (revDecs, simplifyCExp (ctx, env, subst, cont))
          end
        | C.App { applied, cont, args, attr } =>
          ...
          in C.App { applied = applied, cont = cont, args = args, attr = attr }
          end
        ...
        | C.Handle { body, handler = (e, h), successfulExitIn, successfulExitOut } =>
          C.Handle { body = simplifyCExp (ctx, env, subst, body)
...
fun goCExp (ctx : CpsSimplify.Context, exp)
    = let val usage = CpsUsageAnalysis.analyze exp
      ...
      in simplifyCExp (ctx', TypedSyntax.VIdMap.empty, TypedSyntax.VIdMap.empty, exp)
      end
end (* local *)
