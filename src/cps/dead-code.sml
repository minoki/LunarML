...
local
    ...
    in
    fun goSimpleExp (env, _, _, _, C.PrimOp { primOp = _, tyargs = _, args }) = List.app (useValue env) args
      | goSimpleExp (env, _, _, _, C.Record fields) = Syntax.LabelMap.app (useValue env) fields
      | goSimpleExp (_, _, _, _, C.ExnTag { name = _, payloadTy = _ }) = ()
      | goSimpleExp (env, _, _, _, C.Projection { label = _, record, fieldTypes = _ }) = useValue env record
      | goSimpleExp (env, renv, cenv, crenv, C.Abs { contParam, params, body, attr = _ })
...
fun simplifyDec (ctx : Context, appliedCont : C.CVar option) (dec, (env, cenv, subst, csubst, acc : C.Dec list))
    = case dec of
          ...
          in case (exp, results) of
                 ...
                 (case CpsUsageAnalysis.getValueUsage (#usage ctx, result) of
                    ...
                    | _ => let val body = simplifyCExp (ctx, env, cenv, subst, csubst, body)
        ...
        | C.RecDec defs =>
          if List.exists (fn { name, ... } => CpsDeadCodeAnalysis.isUsed (#dead_code_analysis ctx, name)) defs then
              let val defs = List.map (fn { name, contParam, params, body, attr } => { name = name, contParam = contParam, params = params, body = simplifyCExp (ctx, env, cenv, subst, csubst, body), attr = attr }) defs
        ...
        | C.RecContDec defs =>
          if List.all (fn (f, _, _) => CpsUsageAnalysis.getContUsage (#cont_usage ctx, f) = { direct = NEVER, indirect = NEVER }) defs then
              ...
              let val cenv = List.foldl (fn ((name, params, _), cenv) => C.CVarMap.insert (cenv, name, (params, NONE))) cenv defs
                  val dec = C.RecContDec (List.map (fn (name, params, body) => (name, params, simplifyCExp (ctx, env, cenv, subst, csubst, body))) defs)
              in (env, cenv, subst, csubst, dec :: acc)
              end
