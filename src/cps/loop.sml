...
in
structure CpsLoopOptimization : sig
...
fun simplifyDec (ctx : Context) (dec, acc : C.Dec list)
    = case dec of
          ...
          (case (exp, results) of
               ...
               let val body = simplifyCExp (ctx, body)
        ...
        | C.RecDec defs =>
          let val defs = List.map (fn { name, contParam, params, body, attr } => { name = name, contParam = contParam, params = params, body = simplifyCExp (ctx, body), attr = attr }) defs
              fun tryConvertToLoop (def as { name, contParam, params, body, attr })
                  = if C.CVarSet.member (#returnConts (CpsUsageAnalysis.getValueUsage (#rec_usage ctx, name)), contParam) then
                        ...
                        def
          in C.RecDec (List.map tryConvertToLoop defs) :: acc
          end
        ...
        | C.RecContDec defs =>
          let val dec = C.RecContDec (List.map (fn (name, params, body) => (name, params, simplifyCExp (ctx, body))) defs)
          in dec :: acc
          end
