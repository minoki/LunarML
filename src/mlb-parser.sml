...
struct
  structure P =
    ParserCombinator
      (structure Stream = StringStream
       fun showToken c = Char.toString c
       fun showPos {file, line, column} =
         file ^ ":" ^ Int.toString line ^ ":" ^ Int.toString column
       fun comparePos (p: Stream.pos, q: Stream.pos) =
         case String.compare (#file p, #file q) of
           LESS => LESS
         | GREATER => GREATER
         | EQUAL =>
             case Int.compare (#line p, #line q) of
               LESS => LESS
             | GREATER => GREATER
  ...
  val relpath =
    String.concat
    <$> P.many (P.try (arc >>= (fn a => (a ^ "/") <$ CP.char #"/")))
  val abspath = CP.char #"/" >> (fn s => "/" ^ s) <$> relpath
  val path: string P.parser = abspath <|> relpath
  val file: string P.parser = lexeme
    (P.try
       (path
        >>=
        (fn p =>
           filename
           >>= (fn f => P.notFollowedBy (CP.char #"'") >> P.pure (p ^ f)))
        >>=
        (fn file =>
  ...
  val stringLiteral =
        ...
        (CP.char #"\\"
         ...
         (String.str
          ...
          P.choice
            ...
            , #"\r" <$ CP.char #"r"
            , CP.char #"^"
              >> CP.satisfy (fn c => #"\064" <= c andalso c <= #"\095")
              >>= (fn c => P.pure (Char.chr (Char.ord c - 64)))
            , threeDigits
              >>=
              (fn x =>
                 if x <= 255 then P.pure (Char.chr x)
                 else P.fail "escape sequence too large")
            , CP.char #"u" >> fourHexDigits
              >>=
              (fn x =>
                 if x <= 255 then P.pure (Char.chr x)
                 else P.fail "escape sequence too large")
    ...
    in
      lexeme
        (P.between (CP.char #"\"", CP.char #"\"")
           (String.concat <$> P.many elem) <?> "string literal")
    end
  val equal = lexeme (CP.char #"=")
  val basdec = P.fix (fn basdec =>
    let
      val basexp: MLBSyntax.BasExp P.parser = P.fix (fn basexp =>
        P.choice
          [ reserved "bas" >> MLBSyntax.BasisExp <$> basdec <* reserved "end"
          , reserved "let" >> basdec <* reserved "in"
            >>=
            (fn decs =>
               (fn exp => MLBSyntax.LetExp (decs, exp)) <$> basexp
               <* reserved "end")
          , MLBSyntax.BasIdExp <$> identifier
          ])
      val singlebasdec: MLBSyntax.BasDec P.parser = P.choice
        [ reserved "basis"
          >>
          MLBSyntax.BasisDec
          <$>
          P.sepBy1
            ( identifier <* equal
              >>= (fn basid => (fn exp => (basid, exp)) <$> basexp)
            , reserved "and"
            )
        ...
        , reserved "local"
          ...
          (basdec <* reserved "in"
           >>=
           (fn decs1 =>
              (fn decs2 => MLBSyntax.LocalDec (decs1, decs2)) <$> basdec))
          <* reserved "end"
        , reserved "structure"
          >>
          MLBSyntax.StructureDec
          <$>
          P.sepBy1
            ( Syntax.MkStrId <$> identifier
              >>=
              (fn id1 =>
                 (equal >> (fn id2 => (id1, Syntax.MkStrId id2)) <$> identifier)
                 <|> P.pure (id1, id1))
            , reserved "and"
            )
        , reserved "signature"
          >>
          MLBSyntax.SignatureDec
          <$>
          P.sepBy1
            ( Syntax.MkSigId <$> identifier
              >>=
              (fn id1 =>
                 (equal >> (fn id2 => (id1, Syntax.MkSigId id2)) <$> identifier)
                 <|> P.pure (id1, id1))
            , reserved "and"
            )
        , reserved "functor"
          >>
          MLBSyntax.FunctorDec
          <$>
          P.sepBy1
            ( Syntax.MkFunId <$> identifier
              >>=
              (fn id1 =>
                 (equal >> (fn id2 => (id1, Syntax.MkFunId id2)) <$> identifier)
                 <|> P.pure (id1, id1))
            , reserved "and"
            )
        , MLBSyntax.PrimDec <$ reserved "_prim"
        , MLBSyntax.PrimOverloadDec <$ reserved "_primOverloads"
        , reserved "ann" >> P.many1 stringLiteral <* reserved "in"
          >>=
          (fn annotations =>
             (fn dec => MLBSyntax.AnnotationDec (annotations, dec)) <$> basdec
             <* reserved "end")
    ...
    in
      (singlebasdec
       >>=
       (fn dec =>
          ((lexeme (CP.char #";") >> (fn decs => dec :: decs) <$> basdec)
           <|> ((fn decs => dec :: decs) <$> basdec)))) <|> P.pure []
    end)
