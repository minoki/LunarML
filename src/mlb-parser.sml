...
val identifier : string P.parser = lexeme (P.try (CP.letter >>= (fn c0 => P.many (CP.alphaNum <|> CP.oneOf [#"'",#"_"]) >>= (fn cs => P.notFollowedBy pathchar >> let val name = String.implode (c0 :: cs) in if StringSet.member (reservedNames, name) then P.fail "reserved name" else P.pure name end)))) <?> "identifier"
