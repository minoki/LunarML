...
fun charToString (CODEUNIT x) = if x < 0 then
  ...
  | charToString (UNICODE_SCALAR x) = "\\u{" ^ Int.fmt StringCvt.HEX x ^ "}"
fun isAsciiString (s : char vector) : bool = Vector.all (fn CODEUNIT x => 0 <= x andalso x < 128
                                                        | UNICODE_SCALAR x => 0 <= x andalso x < 128) s
fun toAsciiString (s : char vector) : string option
    = let val v = Vector.map (fn CODEUNIT x => if 0 <= x andalso x < 128 then
                                                   Char.chr x
                                               else
                                                   raise Chr
                             | UNICODE_SCALAR x => if 0 <= x andalso x < 128 then
                                                       Char.chr x
                                                   else
                                                       raise Chr
                             ) s
      in SOME (CharVector.tabulate (Vector.length v, fn i => Vector.sub (v, i)))
      end handle Chr => NONE
...
fun encode16bit (s : char vector) : int vector (* may raise Chr *)
    = Vector.fromList (Vector.foldr (fn (CODEUNIT x, xs) => if 0 <= x andalso x <= 0xffff then
                                                                ...
                                                                raise Chr
                                    | (UNICODE_SCALAR x, xs) => if x <= 0xffff then
                                                                    x :: xs
                                                                else
                                                                    let val y = Word.fromInt x - 0wx10000
                                                                        val lead = Word.orb (0wxD800, Word.>> (y, 0w10))
                                                                        val trail = Word.orb (0wxDC00, Word.andb (y, 0wx3ff))
                                                                    in Word.toInt lead :: Word.toInt trail :: xs
                                                                    end
                                    ) [] s
                      )
end;
