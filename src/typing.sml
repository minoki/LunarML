...
fun envToSigEnv(env : Env) : SigEnv
    = { valMap = Syntax.VIdMap.map (fn (tysc, ids, _) => (tysc, ids, ())) (#valMap env)
...
fun freeTyVarsInEnv (_ (* bound *), { valMap = _, tyConMap = _, tyNameMap = _, strMap = _, sigMap = _, funMap = _, boundTyVars } : Env)
...
fun emitError (ctx : Context, spans, message) = let val { matchContext, messageHandler, ... } = ctx
                                                    val message = List.foldl (fn (vid, message) => message ^ " during matching " ^ Syntax.print_VId vid) message matchContext
                                                in Message.error (messageHandler, spans, "type", message)
                                                end
fun emitTypeError (ctx : InferenceContext, spans, message) = emitError (#context ctx, spans, message)
fun emitFatalError (ctx : Context, spans, message) = let val { matchContext, messageHandler, ... } = ctx
                                                         val message = List.foldl (fn (vid, message) => message ^ " during matching " ^ Syntax.print_VId vid) message matchContext
                                                     in Message.fatalError (messageHandler, spans, "type", message)
                                                     end
...
fun lookupStr (_, s : TypedSyntax.Signature, _, nil) = s
  | lookupStr (ctx, { strMap = strMap, ... }, span, (strid0 as Syntax.MkStrId name) :: strids)
...
fun lookupTyConInEnv(ctx, env : ('val,'str) Env', span, Syntax.MkQualified([], tycon as Syntax.MkTyCon name))
    = (case Syntax.TyConMap.find(#tyConMap env, tycon) of
           SOME tystr => tystr
         | NONE => emitFatalError (ctx, [span], "unknown type constructor '" ^ name ^ "'")
      )
  | lookupTyConInEnv(ctx, env, span, Syntax.MkQualified(strid0 :: strids, tycon as Syntax.MkTyCon name))
    = (case Syntax.StrIdMap.find(#strMap env, strid0) of
         ...
         | NONE => emitFatalError (ctx, [span], "unknown structure name '" ^ (case strid0 of Syntax.MkStrId name => name) ^ "'")
      )
fun lookupTyNameInEnv(ctx, { tyNameMap, ... } : ('val,'str) Env', span, tyname)
    = (case TypedSyntax.TyNameMap.find (tyNameMap, tyname) of
           SOME attr => attr
         | NONE => emitFatalError (ctx, [span], "unknown type constructor " ^ TypedSyntax.print_TyName tyname ^ " (internal error)")
      )
...
fun lookupLongVIdInEnv (_ : 'context, env : Env, _ (* span *), longvid as Syntax.MkQualified ([], vid))
    = (case Syntax.VIdMap.find (#valMap env, vid) of
           SOME (tysc, ids, longvid) => Found (longvid, tysc, ids)
         | NONE => ValueNotFound longvid
      )
  | lookupLongVIdInEnv (ctx, env, _, longvid as Syntax.MkQualified (strid0 :: strids, vid))
    = (case Syntax.StrIdMap.find (#strMap env, strid0) of
...
and isConexp (env : Env, TypedSyntax.TypedExp (_, e, _)) = isConexp (env, e)
  ...
  | isConexp (_, TypedSyntax.VarExp (_, TypedSyntax.MkShortVId (TypedSyntax.MkVId (name, _)), Syntax.ValueConstructor _, _)) = name <> "ref"
  | isConexp (_, TypedSyntax.VarExp (_, TypedSyntax.MkLongVId (_, _, Syntax.MkVId name), Syntax.ValueConstructor _, _)) = name <> "ref"
  | isConexp (_, TypedSyntax.VarExp (_, _, Syntax.ExceptionConstructor, _)) = true
  | isConexp (_, _) = false
...
fun isExhaustive (_, _ : Env, TypedSyntax.WildcardPat _) = true
  ...
  | isExhaustive (ctx, env, TypedSyntax.RecordPat { sourceSpan = _, fields, ellipsis = NONE, wholeRecordType = _ }) = List.all (fn (_, e) => isExhaustive (ctx, env, e)) fields
  | isExhaustive (ctx, env, TypedSyntax.RecordPat { sourceSpan = _, fields, ellipsis = SOME ellipsisPat, wholeRecordType = _ }) = List.all (fn (_, e) => isExhaustive (ctx, env, e)) fields andalso isExhaustive (ctx, env, ellipsisPat)
  | isExhaustive (_, _, TypedSyntax.ConPat { sourceSpan = _, longvid = _, payload = NONE, tyargs = _, valueConstructorInfo = SOME info }) = Syntax.VIdSet.numItems (#allConstructors info) = 1
  | isExhaustive (ctx, env, TypedSyntax.ConPat { sourceSpan = _, longvid = _, payload = SOME (_, innerPat), tyargs = _, valueConstructorInfo = SOME info }) = Syntax.VIdSet.numItems (#allConstructors info) = 1 andalso isExhaustive (ctx, env, innerPat)
  | isExhaustive (_, _, TypedSyntax.ConPat { sourceSpan = _, longvid = _, payload = _, tyargs = _, valueConstructorInfo = NONE }) = false
  ...
  | isExhaustive (_, _, TypedSyntax.VectorPat (_, pats, ellipsis, _)) = ellipsis andalso Vector.length pats = 0
...
val primTy_char16 = TypedSyntax.TyCon (SourcePos.nullSpan, [], primTyName_char16)
val primTy_string16 = TypedSyntax.TyCon (SourcePos.nullSpan, [], primTyName_string16)
...
fun freshTyVar (ctx : InferenceContext, span : SourcePos.span, equality : bool, class : TypedSyntax.class option) : TypedSyntax.AnonymousTyVar
    = ref (TypedSyntax.Unbound ({ sourceSpan = span, equalityRequired = equality, class = class }, #level ctx))
...
fun occurCheckAndAdjustLevel tv
    = let fun check (T.AnonymousTyVar (_, tv'))
              = if tv = tv' then
                    ...
                    (case !tv' of
                         T.Unbound (ct, l') =>
                         let val minLevel = case !tv of
                                                T.Unbound (_, l) => Int.min (l, l')
                                              | _ => l'
                         in tv' := T.Unbound (ct, minLevel)
                          ; false
                         end
                       | T.Link ty => check ty
                    )
...
fun instantiate (ctx : InferenceContext, span, T.TypeScheme (vars, ty))
    = let val (subst, tyargs) = List.foldl (fn ((v, ct), (set, rest)) =>
                                               let val (equality, class) = case ct of
                                                                               NONE => (false, NONE)
                                                                             | SOME (T.IsRecord excluded) => (false, SOME (T.Record (List.foldl Syntax.LabelSet.add' Syntax.LabelSet.empty excluded)))
                                               ...
                                               in (T.TyVarMap.insert (set, v, tyarg), (tyarg, ct) :: rest)
                                               end
                                           ) (T.TyVarMap.empty, []) vars
      in (applySubstTy subst ty, List.rev tyargs)
      end

structure ConstraintInfo : sig
              datatype constraint_place = SEQUENCE | BRANCH
              datatype constraint_info = EXPECTED_ACTUAL
                                       | ACTUAL_EXPECTED
                                       | PREVIOUS_CURRENT of constraint_place
                                       | CURRENT_PREVIOUS of constraint_place
              val flip : constraint_info -> constraint_info
              datatype 'a sorted = SUBSUMPTION of { expected : 'a, actual : 'a }
                                 | SEQUENCE_OR_BRANCH of { previous : 'a, current : 'a, place : constraint_place }
...
fun sort (EXPECTED_ACTUAL, a, b) = SUBSUMPTION { expected = a, actual = b }
  ...
  | sort (PREVIOUS_CURRENT place, a, b) = SEQUENCE_OR_BRANCH { previous = a, current = b, place = place }
  | sort (CURRENT_PREVIOUS place, a, b) = SEQUENCE_OR_BRANCH { previous = a, current = b, place = place }
end
...
fun unify (ctx : InferenceContext, env : Env, ci, span1, T.AnonymousTyVar (_, tv), ty) = unifyTyVarAndTy (ctx, env, ci, span1, tv, ty)
  | unify (ctx, env, ci, span1, ty, T.AnonymousTyVar (_, tv)) = unifyTyVarAndTy (ctx, env, ConstraintInfo.flip ci, span1, tv, ty)
  | unify (ctx, _, ci, span1, T.TyVar (_, tv as T.MkTyVar (name, _)), T.TyVar (_, tv' as T.MkTyVar (name', _)))
    = if T.eqUTyVar (tv, tv') then
          () (* do nothing *)
      else
          (case ConstraintInfo.sort (ci, name, name') of
               ConstraintInfo.SUBSUMPTION { expected, actual } => emitTypeError (ctx, [span1], "expected " ^ expected ^ ", but got " ^ actual)
             | ConstraintInfo.SEQUENCE_OR_BRANCH { previous, current, place = _ } => emitTypeError (ctx, [span1], "previous was " ^ previous ^ ", but now is " ^ current)
          )
  | unify (ctx, env, ci, span, T.FnType (_, s0, s1), T.FnType (_, t0, t1)) = (unify (ctx, env, ci, span, s0, t0); unify (ctx, env, ci, span, s1, t1))
  | unify (ctx, env, ci, span1, T.RecordType (span2, fields), T.RecordType (span3, fields'))
    = let val incompatible = Syntax.LabelMap.numItems fields <> Syntax.LabelMap.numItems fields'
          val incompatible = Syntax.LabelMap.foldli (fn (label, ty, incompatible) =>
                                                        case Syntax.LabelMap.find (fields', label) of
                                                            NONE => true
                                                          | SOME ty' => (unify (ctx, env, ci, span1, ty, ty'); incompatible)
                                                    ) incompatible fields
  ...
  | unify (ctx, env, ci, span1, T.RecordType (span2, fields), T.RecordExtType (span3, fields', baseTy))
    = let val incompatible = Syntax.LabelMap.numItems fields < Syntax.LabelMap.numItems fields'
          val incompatible = Syntax.LabelMap.foldli (fn (label, ty, incompatible) =>
                                                        case Syntax.LabelMap.find (fields, label) of
                                                            NONE => true
      ...
      in unify (ctx, env, ci, span1, T.RecordType (span2, extraFields), baseTy)
       ; if incompatible then
             emitTypeError (ctx, [span1, span2, span3], "unification failed: incompatible record types (different fields)")
         else
             ()
      end
  | unify (ctx, env, ci, span1, T.RecordExtType (span3, fields', baseTy), T.RecordType (span2, fields))
    = let val incompatible = Syntax.LabelMap.numItems fields < Syntax.LabelMap.numItems fields'
          val incompatible = Syntax.LabelMap.foldli (fn (label, ty, incompatible) =>
                                                        case Syntax.LabelMap.find (fields, label) of
                                                            NONE => true
          ...
          val extraFields = Syntax.LabelMap.filteri (fn (label, _) => not (Syntax.LabelMap.inDomain (fields', label))) fields
      in unify (ctx, env, ci, span1, baseTy, T.RecordType (span2, extraFields))
       ; if incompatible then
             emitTypeError (ctx, [span1, span2, span3], "unification failed: incompatible record types (different fields)")
         else
             ()
      end
  | unify (ctx, env, ci, span1, T.RecordExtType (span2, fields, baseTy), T.RecordExtType (span3, fields', baseTy'))
    = let val () = Syntax.LabelMap.app (fn (ty, ty') => unify (ctx, env, ci, span1, ty, ty')) (Syntax.LabelMap.intersectWith (fn (ty, ty') => (ty, ty')) (fields, fields'))
      ...
      in case (Syntax.LabelMap.isEmpty uniqueFields, Syntax.LabelMap.isEmpty uniqueFields') of
           ...
           | (true, false) => unify (ctx, env, ci, span1, baseTy, T.RecordExtType (span3, uniqueFields', baseTy')) (* fields is a proper submap of fields' *)
           | (false, true) => unify (ctx, env, ci, span1, T.RecordExtType (span3, uniqueFields, baseTy), baseTy') (* fields' is a proper submap of fields *)
           | (false, false) => let val unionFields = Syntax.LabelMap.foldli (fn (label, _, acc) => Syntax.LabelSet.add (acc, label)) (Syntax.LabelMap.foldli (fn (label, _, acc) => Syntax.LabelSet.add (acc, label)) Syntax.LabelSet.empty fields) fields'
                                   val commonBaseTy = T.AnonymousTyVar (span1, freshTyVar (ctx, span1, false, SOME (T.Record unionFields)))
                               in unify (ctx, env, ci, span1, baseTy, T.RecordExtType (span3, uniqueFields', commonBaseTy))
                                ; unify (ctx, env, ci, span1, baseTy', T.RecordExtType (span2, uniqueFields, commonBaseTy))
                               end
      end
  | unify (ctx, env, ci, span1, ty1 as T.TyCon (span2, tyarg, con), ty2 as T.TyCon (span3, tyarg', con'))
    = if T.eqTyName (con, con') then
          ListPair.appEq (fn (x, y) => unify (ctx, env, ci, span1, x, y)) (tyarg, tyarg')
          ...
          (case ConstraintInfo.sort (ci, ty1, ty2) of
               ConstraintInfo.SUBSUMPTION { expected, actual } => emitTypeError (ctx, [span1], "expected " ^ TypedSyntax.PrettyPrint.print_Ty expected ^ ", but got " ^ TypedSyntax.PrettyPrint.print_Ty actual)
             | ConstraintInfo.SEQUENCE_OR_BRANCH { previous, current, place = _ } => emitTypeError (ctx, [span1], "previous was " ^ TypedSyntax.PrettyPrint.print_Ty previous ^ ", but now is " ^ TypedSyntax.PrettyPrint.print_Ty current)
          )
  ...
  | unify (ctx, _, ci, span, ty1, ty2)
    = (case ConstraintInfo.sort (ci, ty1, ty2) of
           ConstraintInfo.SUBSUMPTION { expected, actual } => emitTypeError (ctx, [span], "expected " ^ TypedSyntax.PrettyPrint.print_Ty expected ^ ", but got " ^ TypedSyntax.PrettyPrint.print_Ty actual)
         | ConstraintInfo.SEQUENCE_OR_BRANCH { previous, current, place = _ } => emitTypeError (ctx, [span], "previous was " ^ TypedSyntax.PrettyPrint.print_Ty previous ^ ", but now is " ^ TypedSyntax.PrettyPrint.print_Ty current)
      )
and solveUnary (ctx : InferenceContext, env : Env, ci, span1, ty, ct)
    = let fun mismatch ()
              = let val lhs = case ty of
                    ...
                    val rhs = case ct of
                                ...
                                | T.IsChar => "char"
                                | T.IsString => "string"
                in case ConstraintInfo.sort (ci, lhs, rhs) of
                       ConstraintInfo.SUBSUMPTION { expected, actual } => emitTypeError (ctx, [span1], "expected " ^ expected ^ ", but got " ^ actual)
                     | ConstraintInfo.SEQUENCE_OR_BRANCH { previous, current, place = _ } => emitTypeError (ctx, [span1], "previous was " ^ previous ^ ", but now is " ^ current)
                end
      in case (ty, ct) of
           ...
           | (T.RecordExtType (span2, fields, baseTy), T.IsRecord labels) =>
             ...
             in solveUnary (ctx, env, ci, span1, baseTy, T.IsRecord baseExcludedLabels)
             end
           | (T.TyCon _, T.IsRecord _) => mismatch ()
           | (T.FnType _, T.IsRecord _) => mismatch ()
           | (T.TyVar _, T.IsRecord _) => mismatch ()
           | (T.RecordType (_, fields), T.IsEqType) => Syntax.LabelMap.app (fn ty => solveUnary (ctx, env, ci, span1, ty, T.IsEqType)) fields
           ...
           | (T.RecordExtType (_, fields, baseTy), T.IsEqType) =>
             ( Syntax.LabelMap.app (fn ty => solveUnary (ctx, env, ci, span1, ty, T.IsEqType)) fields
           ...
           | (T.TyCon (span2, tyargs, tyname), T.IsEqType) =>
             ...
             in if isRefOrArray tyname then
                    ...
                    List.app (fn tyarg => solveUnary (ctx, env, ci, span1, tyarg, T.IsEqType)) tyargs
                else
           ...
           | (T.AnonymousTyVar (_, tv), pred) =>
             (case !tv of
                  T.Link replacement => solveUnary (ctx, env, ci, span1, replacement, pred)
                | T.Unbound ({ sourceSpan, equalityRequired, class }, level) =>
                  let val class' = case (pred, class) of
                                       (T.IsRecord labels, NONE) => SOME (T.Record (List.foldl Syntax.LabelSet.add' Syntax.LabelSet.empty labels))
                                     | (T.IsRecord labels, SOME (T.Record labels')) => SOME (T.Record (List.foldl Syntax.LabelSet.add' labels' labels))
                                     | (T.IsEqType, _) => class
                      ...
                      val equalityRequired' = case (pred, class') of
                                                  (T.IsEqType, SOME T.Real) =>
                                                  ( emitTypeError (ctx, [span1], "real does not admit equality")
                                                  ; true
                                                  )
                                                | (T.IsEqType, _) => true
                                                | _ => equalityRequired
                  in tv := T.Unbound ({ sourceSpan = sourceSpan, equalityRequired = equalityRequired', class = class' }, level)
                  end
             )
      end
and unifyTyVarAndTy (ctx : InferenceContext, env : Env, ci : ConstraintInfo.constraint_info, span : SourcePos.span, tv : T.AnonymousTyVar, ty : T.Ty) : unit
    = (case !tv of
         ...
         | T.Unbound ({ sourceSpan = _, equalityRequired, class }, _) =>
           ...
           in if (case ty of T.AnonymousTyVar (_, tv') => tv = tv' | _ => false) then (* ty = AnonymousTyVar tv *)
                  ...
                  let val ci' = ConstraintInfo.flip ci
                  in tv := T.Link ty
                   ...
                   ; case class of
                         NONE => ()
                       | SOME (T.Record excludedLabels) => solveUnary (ctx, env, ci', span, ty, T.IsRecord (Syntax.LabelSet.toList excludedLabels))
                       | SOME T.NumTxt => solveUnary (ctx, env, ci', span, ty, T.IsOrdered)
                       | SOME T.IntWordReal => solveUnary (ctx, env, ci', span, ty, T.IsRing)
                       | SOME T.IntWord => solveUnary (ctx, env, ci', span, ty, T.IsIntegral)
                       | SOME T.IntReal => solveUnary (ctx, env, ci', span, ty, T.IsSignedReal)
                       ...
                       | SOME T.String => solveUnary (ctx, env, ci', span, ty, T.IsString)
                  end
           end
      )
...
fun commonType (ctx : InferenceContext, env : Env, place, span, previousTy, currentTy)
    = ( unify (ctx, env, ConstraintInfo.PREVIOUS_CURRENT place, span, previousTy, currentTy)
      ; previousTy
      )
...
fun evalTy (ctx : Context, env : ('val,'str) Env', S.TyVar (span, tv)) : T.Ty
    = (case Syntax.TyVarMap.find(#boundTyVars env, tv) of
         ...
         | NONE => emitFatalError (ctx, [span], "unknown type varibale `" ^ Syntax.print_TyVar tv ^ "`")
      )
  ...
  | evalTy (ctx, env, S.TyCon (span, args, tycon))
    = let val { typeFunction = T.TypeFunction (tyvars, ty), ... } = lookupTyConInEnv (ctx, env, span, tycon)
          val subst = (ListPair.foldlEq (fn (tv, arg, m) => TypedSyntax.TyVarMap.insert (m, tv, evalTy (ctx, env, arg))) TypedSyntax.TyVarMap.empty (tyvars, args))
                      handle ListPair.UnequalLengths => emitFatalError (ctx, [span], "invalid type construction")
...
fun forceTy (T.AnonymousTyVar (_, ref (T.Link ty))) = forceTy ty
  | forceTy (ty as T.RecordExtType (span, fieldTypes, baseTy))
    = let val baseTy = forceTy baseTy
      in case baseTy of
             T.RecordType (_, fieldTypes') => T.RecordType (span, Syntax.LabelMap.unionWith #1 (fieldTypes, fieldTypes'))
           | _ => ty
      end
  | forceTy ty = ty
 ...
 *)
fun synthTypeOfPat (ctx : InferenceContext, _ : Env, S.WildcardPat span) : T.Ty * (T.VId * T.Ty) S.VIdMap.map * T.Pat
    = let val ty = T.AnonymousTyVar (span, freshTyVar (ctx, span, false, NONE))
      in (ty, S.VIdMap.empty, T.WildcardPat span)
      end
  | synthTypeOfPat (ctx, _, S.SConPat (span, scon))
    = (case scon of
           Syntax.IntegerConstant _   => let val tv = freshTyVar (ctx, span, true, SOME T.Int)
                                             val ty = T.AnonymousTyVar (span, tv)
                                         in (ty, S.VIdMap.empty, T.SConPat (span, scon, ty))
                                         end
         | Syntax.WordConstant _      => let val tv = freshTyVar (ctx, span, true, SOME T.Word)
                                             val ty = T.AnonymousTyVar (span, tv)
                                         in (ty, S.VIdMap.empty, T.SConPat (span, scon, ty))
                                         end
         | Syntax.RealConstant _      => emitFatalTypeError (ctx, [span], "no real constant may occur in a pattern")
         | Syntax.CharacterConstant _ => let val tv = freshTyVar (ctx, span, true, SOME T.Char)
                                             val ty = T.AnonymousTyVar (span, tv)
                                         in (ty, S.VIdMap.empty, T.SConPat (span, scon, ty))
                                         end
         | Syntax.StringConstant _    => let val tv = freshTyVar (ctx, span, true, SOME T.String)
                                             val ty = T.AnonymousTyVar (span, tv)
                                         in (ty, S.VIdMap.empty, T.SConPat (span, scon, ty))
                                         end
      )
  | synthTypeOfPat (ctx, _, S.VarPat (span, vid as S.MkVId "_Prim.ref"))
    = let val payloadTy = TypedSyntax.AnonymousTyVar (span, freshTyVar (ctx, span, false, NONE))
  ...
  | synthTypeOfPat (ctx, _, S.VarPat (span, vid as S.MkVId "_Prim.::"))
    = let val elemTy = TypedSyntax.AnonymousTyVar (span, freshTyVar (ctx, span, false, NONE))
  ...
  | synthTypeOfPat (_, _, S.VarPat (span, vid as S.MkVId "_Prim.unit.equal"))
  ...
  | synthTypeOfPat (ctx, env, S.VarPat (span, vid))
    = (case Syntax.VIdMap.find (#valMap env, vid) of
           SOME (_, Syntax.ValueConstructor _, _) => emitFatalTypeError (ctx, [span], "VarPat: invalid pattern")
         | SOME (_, Syntax.ExceptionConstructor, _) => emitFatalTypeError (ctx, [span], "VarPat: invalid pattern")
         | _ => let val ty = TypedSyntax.AnonymousTyVar (span, freshTyVar (ctx, span, false, NONE))
  ...
  | synthTypeOfPat (ctx, env, S.RecordPat { sourceSpan, fields, ellipsis })
    = let fun oneField ((label, pat), (fieldTypes, vars, fieldPats))
              = let val (ty, vars', pat') = synthTypeOfPat (ctx, env, pat)
                in (Syntax.LabelMap.insert (fieldTypes, label, ty), Syntax.VIdMap.unionWith (fn (_, y) => (emitTypeError (ctx, [sourceSpan], "duplicate identifier in a pattern"); y)) (vars, vars'), (label, pat') :: fieldPats)
                end
          val (fieldTypes, vars, fieldPats) = List.foldr oneField (Syntax.LabelMap.empty, Syntax.VIdMap.empty, []) fields
      in case ellipsis of
             SOME basePat =>
             let val span = S.getSourceSpanOfPat basePat
                 val (baseTy, vars', basePat) = synthTypeOfPat (ctx, env, basePat)
                 val recordTy = T.RecordExtType (sourceSpan, fieldTypes, baseTy)
                 val () = solveUnary (ctx, env, ConstraintInfo.ACTUAL_EXPECTED, span, baseTy, T.IsRecord (List.map #1 fields))
                ...
                in case optInnerPat of
                     ...
                     | SOME innerPat =>
                       (case ty of
                            ...
                            let val (innerVars, innerPat') = checkTypeOfPat (ctx, env, innerPat, argTy)
                            in (resultTy, innerVars, T.ConPat { sourceSpan = span, longvid = longvid, payload = SOME (argTy, innerPat'), tyargs = List.map #1 tyargs, valueConstructorInfo = valueConstructorInfo })
                            end
                          | _ => emitFatalTypeError (ctx, [span], "invalid pattern")
                       )
                end
            else (* idstatus = Syntax.ValueVariable *)
                emitFatalTypeError (ctx, [span], "invalid pattern")
           )
         | ValueNotFound notfound => emitFatalTypeError (ctx, [span], "invalid pattern: value name '" ^ Syntax.print_LongVId notfound ^ "' not found")
         | StructureNotFound notfound => emitFatalTypeError (ctx, [span], "invalid pattern: structure name '" ^ Syntax.print_LongStrId notfound ^ "' not found")
      )
  | synthTypeOfPat (ctx, env, S.TypedPat (span, pat, ty))
    = let val ty = evalTy (#context ctx, env, ty)
      ...
      in (ty, vars, T.TypedPat (span, pat, ty))
      end
  | synthTypeOfPat (ctx, env, S.LayeredPat (span, vid, NONE, pat))
    = let val (ty, vars, pat) = synthTypeOfPat (ctx, env, pat)
          val () = case Syntax.VIdMap.find (vars, vid) of
                       NONE => ()
...
and checkTypeOfPat (_ : InferenceContext, _ : Env, S.WildcardPat span, _) : (T.VId * T.Ty) S.VIdMap.map * T.Pat
  ...
  | checkTypeOfPat (ctx, env, S.SConPat (span, scon), expectedTy)
    = (case scon of
           Syntax.IntegerConstant _   => ( solveUnary (ctx, env, ConstraintInfo.EXPECTED_ACTUAL, span, expectedTy, T.IsInt)
                                         ; solveUnary (ctx, env, ConstraintInfo.EXPECTED_ACTUAL, span, expectedTy, T.IsEqType)
                                         ; (S.VIdMap.empty, T.SConPat (span, scon, expectedTy))
                                         )
         | Syntax.WordConstant _      => ( solveUnary (ctx, env, ConstraintInfo.EXPECTED_ACTUAL, span, expectedTy, T.IsWord)
                                         ; solveUnary (ctx, env, ConstraintInfo.EXPECTED_ACTUAL, span, expectedTy, T.IsEqType)
                                         ; (S.VIdMap.empty, T.SConPat (span, scon, expectedTy))
                                         )
         | Syntax.RealConstant _      => emitFatalTypeError (ctx, [span], "no real constant may occur in a pattern")
         | Syntax.CharacterConstant _ => ( solveUnary (ctx, env, ConstraintInfo.EXPECTED_ACTUAL, span, expectedTy, T.IsChar)
                                         ; solveUnary (ctx, env, ConstraintInfo.EXPECTED_ACTUAL, span, expectedTy, T.IsEqType)
                                         ; (S.VIdMap.empty, T.SConPat (span, scon, expectedTy))
                                         )
         | Syntax.StringConstant _    => ( solveUnary (ctx, env, ConstraintInfo.EXPECTED_ACTUAL, span, expectedTy, T.IsString)
                                         ; solveUnary (ctx, env, ConstraintInfo.EXPECTED_ACTUAL, span, expectedTy, T.IsEqType)
                                         ; (S.VIdMap.empty, T.SConPat (span, scon, expectedTy))
                                         )
      )
  ...
  | checkTypeOfPat (ctx, env, S.VarPat (span, vid), expectedTy)
    = (case Syntax.VIdMap.find (#valMap env, vid) of
           SOME (_, Syntax.ValueConstructor _, _) => emitFatalTypeError (ctx, [span], "VarPat: invalid pattern")
         | SOME (_, Syntax.ExceptionConstructor, _) => emitFatalTypeError (ctx, [span], "VarPat: invalid pattern")
         | _ => let val vid' = newVId (#context ctx, vid)
                in (S.VIdMap.singleton (vid, (vid', expectedTy)), T.VarPat (span, vid', expectedTy))
                end
      )
  | checkTypeOfPat (ctx, env, pat as S.RecordPat { sourceSpan, fields, ellipsis }, expectedTy)
    = (case forceTy expectedTy of
           T.RecordType (span', fieldTypes) =>
           let fun oneField ((label, pat), (vars, fieldPats))
                   = case Syntax.LabelMap.find (fieldTypes, label) of
                         SOME ty => let val (vars', pat') = checkTypeOfPat (ctx, env, pat, ty)
                                    in (Syntax.VIdMap.unionWith (fn (_, y) => (emitTypeError (ctx, [sourceSpan], "duplicate identifier in a pattern"); y)) (vars, vars'), (label, pat') :: fieldPats)
                                    end
                       | NONE => ( emitTypeError (ctx, [sourceSpan], "expected " ^ Syntax.print_Label label ^ " to be present")
                                 ; (vars, fieldPats)
                                 )
               val (vars, fieldPats) = List.foldr oneField (Syntax.VIdMap.empty, []) fields
               val baseFields = List.foldl (fn ((label, _), map) => if Syntax.LabelMap.inDomain (map, label) then
                                                                        #1 (Syntax.LabelMap.remove (map, label))
                                                                    else
                                                                        map) fieldTypes fields
           in case ellipsis of
                  SOME basePat =>
                  let val (vars', basePat) = checkTypeOfPat (ctx, env, basePat, T.RecordType (span', baseFields))
                      val vars'' = Syntax.VIdMap.unionWith (fn (_, y) => (emitTypeError (ctx, [sourceSpan], "duplicate identifier in a pattern"); y)) (vars, vars')
                  in (vars'', T.RecordPat { sourceSpan = sourceSpan, fields = fieldPats, ellipsis = SOME basePat, wholeRecordType = expectedTy })
                  end
                | NONE => ( if Syntax.LabelMap.isEmpty baseFields then
                                ()
                            else
                                emitTypeError (ctx, [sourceSpan], "missing fields: " ^ Syntax.print_list Syntax.print_Label (Syntax.LabelMap.foldri (fn (label, _, acc) => label :: acc) [] baseFields))
                          ; (vars, T.RecordPat { sourceSpan = sourceSpan, fields = fieldPats, ellipsis = NONE, wholeRecordType = expectedTy })
                          )
           end
         | _ =>
  ...
  | checkTypeOfPat (ctx, env, S.LayeredPat (span, vid, NONE, pat), expectedTy)
    = let val (vars, pat) = checkTypeOfPat (ctx, env, pat, expectedTy)
          val () = case Syntax.VIdMap.find (vars, vid) of
                       NONE => ()
                     | SOME _ => emitTypeError (ctx, [span], "duplicate identifier in a pattern")
          val vid' = newVId (#context ctx, vid)
      in (Syntax.VIdMap.insert (vars, vid, (vid', expectedTy)), T.LayeredPat (span, vid', expectedTy, pat))
      end
  | checkTypeOfPat (ctx, env, pat as S.VectorPat (span, pats, ellipsis), expectedTy)
    = (case forceTy expectedTy of
           T.TyCon (_, [elemTy], con) =>
           if T.eqTyName (con, primTyName_vector) then
               let val varsAndPats = Vector.map (fn pat => checkTypeOfPat (ctx, env, pat, elemTy)) pats
                   val vars = Vector.foldr (fn ((vars, _), vars') => Syntax.VIdMap.unionWith (fn (_, y) => (emitTypeError (ctx, [], "duplicate identifier in a pattern"); y)) (vars, vars')) Syntax.VIdMap.empty varsAndPats
                   val pats = Vector.map #2 varsAndPats
               in (vars, T.VectorPat (span, pats, ellipsis, elemTy))
               end
           else
               let val (actualTy, map, pat) = synthTypeOfPat (ctx, env, pat)
               in checkSubsumption (ctx, env, span, actualTy, expectedTy)
...
fun doWithtype (ctx, _ (* Env *), typbinds : S.TypBind list) : S.ConBind -> S.ConBind
    = let val map = List.foldl (fn (S.TypBind (_, tyvars, tycon, ty), map) => S.TyConMap.insert (map, tycon, (tyvars, ty))) S.TyConMap.empty typbinds
          fun goTyAlias env (ty as S.TyVar (_, tv)) = (case S.TyVarMap.find (env, tv) of
                                                           SOME ty => ty
                                                         | NONE => ty
                                                      )
            ...
            | goTy (S.TyCon (span, tyargs, Syntax.MkQualified ([], tycon))) = let val tyargs = List.map goTy tyargs
                                                                              in case S.TyConMap.find (map, tycon) of
                                                                                     SOME (tyvars, ty) => let val env' = ListPair.foldlEq (fn (tv, tyarg, m) => S.TyVarMap.insert (m, tv, tyarg)) S.TyVarMap.empty (tyvars, tyargs)
                                                                                                                         handle ListPair.UnequalLengths => emitFatalError (ctx, [span], "invalid type construction: arity mismatch")
                                                                                                          in goTyAlias env' ty
                                                                                                          end
...
fun determineDatatypeEquality(ctx, env : ('val,'str) Env', datbinds : (S.TyVar list * S.Ty list) S.TyConMap.map) : bool S.TyConMap.map
    = let val localTyCons = S.TyConMap.foldli (fn (tycon, _, s) => S.TyConSet.add (s, tycon)) S.TyConSet.empty datbinds
          ...
          fun doDatBind (tycon, (tyvars, payloads))
              = let fun doTy (S.TyVar (_, tv)) = if List.exists (fn tv' => tv = tv') tyvars then
                                                     SOME []
                                                 else
                                                     (case tv of
                      ...
                      | doTy (S.RecordType (_, fields, NONE)) = doTypes (List.map #2 fields)
                      | doTy (S.RecordType (_, fields, SOME baseTy)) = (case doTypes (List.map #2 fields) of
                        ...
                        = let val l = case longtycon of
                          ...
                          in case l of
                               ...
                               | NONE => let val { typeFunction = T.TypeFunction (tyvars, ty), ... } = lookupTyConInEnv (ctx, env, span, longtycon)
                                             ...
                                             fun doUTy (T.TyVar (_, tv)) = (case T.TyVarMap.find (tyVarMap, tv) of
                                                                                SOME ty => doTy ty
                                               ...
                                               | doUTy (T.RecordType (_, fields)) = doUTypes (Syntax.LabelMap.foldl (op ::) [] fields)
                                               | doUTy (T.RecordExtType (_, fields, baseTy)) = doUTypes (Syntax.LabelMap.foldl (op ::) [baseTy] fields) (* should not occur *)
 ...
 *)
fun synthTypeOfExp (ctx : InferenceContext, _ : Env, S.SConExp (span, scon)) : T.Ty * T.Exp
    = let val ty = case scon of
                       Syntax.IntegerConstant _   => let val tv = freshTyVar (ctx, span, false, SOME T.Int)
                                                     in T.AnonymousTyVar (span, tv)
                                                     end
                     | Syntax.WordConstant _      => let val tv = freshTyVar (ctx, span, false, SOME T.Word)
                                                     in T.AnonymousTyVar (span, tv)
                                                     end
                     | Syntax.RealConstant _      => primTy_real (* TODO: overloaded literals *)
                     | Syntax.CharacterConstant _ => let val tv = freshTyVar (ctx, span, false, SOME T.Char)
                                                     in T.AnonymousTyVar (span, tv)
                                                     end
                     | Syntax.StringConstant _    => let val tv = freshTyVar (ctx, span, false, SOME T.String)
                                                     in T.AnonymousTyVar (span, tv)
                                                     end
  ...
  | synthTypeOfExp (ctx, env, S.VarExp (span, longvid))
    = (case lookupLongVIdInEnv(ctx, env, span, longvid) of
         ...
         | ValueNotFound notfound => let val () = emitTypeError (ctx, [span], "unknown value name " ^ Syntax.print_LongVId notfound)
                                         val ty = T.AnonymousTyVar (span, freshTyVar (ctx, span, false, NONE))
  ...
  | synthTypeOfExp (ctx, env, S.RecordExp (span, fields, NONE))
    = let fun doField ((label, exp), (typeMap, expList))
      ...
      in (T.RecordType (span, fieldTypes), T.RecordExp (span, List.rev revFields))
      end
  | synthTypeOfExp (ctx, env, S.RecordExp (span, fields, SOME baseExp))
    = let val baseSpan = S.getSourceSpanOfExp baseExp
          val (baseTy, baseExp) = synthTypeOfExp (ctx, env, baseExp)
          val () = solveUnary (ctx, env, ConstraintInfo.ACTUAL_EXPECTED, baseSpan, baseTy, T.IsRecord (List.map #1 fields))
          fun doField ((label, exp), (typeMap, expList))
              = let val (ty, exp) = synthTypeOfExp (ctx, env, exp)
                in (Syntax.LabelMap.insert (typeMap, label, ty), (label, exp) :: expList)
                end
          val (fieldTypes, revFields) = List.foldl doField (Syntax.LabelMap.empty, []) fields
          val recordTy = case forceTy baseTy of
                             T.RecordType (span, fieldTypes') => T.RecordType (span, Syntax.LabelMap.unionWith #1 (fieldTypes, fieldTypes'))
                           | baseTy => T.RecordExtType (span, fieldTypes, baseTy)
      in (recordTy, T.RecordExtExp { sourceSpan = span, fields = List.rev revFields, baseExp = baseExp, baseTy = baseTy })
      end
  | synthTypeOfExp (ctx, env, S.LetInExp (span, decs, innerExp))
    = let val (env', decs) = typeCheckDecs (ctx, env, decs)
          val (ty, innerExp) = synthTypeOfExp (ctx, mergeEnv (env, env'), innerExp)
      in (ty, T.LetInExp (span, decs, innerExp))
      end
  | synthTypeOfExp (ctx, env, S.AppExp (span, f, arg))
    (* TODO: special case for projection *)
    = let val (funTy, f) = synthTypeOfExp (ctx, env, f)
      in case forceTy funTy of
             T.FnType (_, argTy, resultTy) =>
             (resultTy, T.AppExp (span, f, checkTypeOfExp (ctx, env, arg, argTy)))
           | funTy =>
             let val (argTy, arg) = synthTypeOfExp (ctx, env, arg)
                 val resultTy = T.AnonymousTyVar (span, freshTyVar (ctx, span, false, NONE))
  ...
  | synthTypeOfExp (ctx, env, S.TypedExp (span, exp, ty))
    = let val ty = evalTy (#context ctx, env, ty)
      ...
      in (ty, T.TypedExp (span, exp, ty))
      end
  | synthTypeOfExp (ctx, env, S.HandleExp (span, exp, matches))
    (* exp : t, matches : exn -> t *)
    = let val (expTy, exp) = synthTypeOfExp (ctx, env, exp)
          val matches = checkTypeOfMatch (ctx, env, matches, primTy_exn, expTy)
      in (expTy, T.HandleExp (span, exp, matches, expTy))
      end
  | synthTypeOfExp (ctx, env, S.RaiseExp (span, exp))
    = let val exp = checkTypeOfExp (ctx, env, exp, primTy_exn)
          val resultTy = T.AnonymousTyVar (span, freshTyVar (ctx, span, false, NONE))
      in (resultTy, T.RaiseExp (span, resultTy, exp))
      end
  | synthTypeOfExp (ctx, env, S.IfThenElseExp (span, cond, thenPart, elsePart))
    = let val elseSpan = S.getSourceSpanOfExp elsePart
          ...
          val resultTy = commonType (ctx, env, ConstraintInfo.BRANCH, elseSpan, thenTy, elseTy)
      in (resultTy, T.IfThenElseExp (span, cond, thenPart, elsePart))
      end
  | synthTypeOfExp (ctx, env, S.CaseExp (span, exp, matches))
    = let val (expTy, exp) = synthTypeOfExp (ctx, env, exp)
          val (retTy, matches) = checkAndSynthTypeOfMatch (ctx, env, span, matches, expTy)
      in (retTy, T.CaseExp { sourceSpan = span, subjectExp = exp, subjectTy = expTy, matches = matches, matchType = T.CASE, resultTy = retTy })
      end
  | synthTypeOfExp (ctx, env, S.FnExp (span, matches))
    = let val (argTy, retTy, matches) = synthTypeOfMatch (ctx, env, span, matches)
          val fnExp = case matches of
                          [(T.VarPat (_, vid, _), body)] => T.FnExp (span, vid, argTy, body)
  ...
  | synthTypeOfExp (ctx, _, S.ProjectionExp (span, label))
    = let val fieldTy = TypedSyntax.AnonymousTyVar (span, freshTyVar (ctx, span, false, NONE))
          val baseTy = T.AnonymousTyVar (span, freshTyVar (ctx, span, false, SOME (T.Record (Syntax.LabelSet.singleton label))))
  ...
  | synthTypeOfExp (ctx, env, S.PrimExp (span, primOp, tyargs, args))
    = let val () = if Vector.length tyargs = 0 then
          ...
          val (subst, tyargs) = List.foldr (fn ((v, equality), (m, rest)) =>
                                               let val tv = freshTyVar (ctx, span, equality, NONE)
                                                   val tyarg = T.AnonymousTyVar (span, tv)
          ...
          val args = Vector.mapi (fn (i, argTy) => checkTypeOfExp (ctx, env, Vector.sub (args, i), argTy)) argTypes
...
and checkTypeOfExp (ctx, env, S.SConExp (span, scon), expectedTy : T.Ty) : T.Exp
  ...
  | checkTypeOfExp (ctx, env, exp as S.RecordExp (span, fields, SOME baseExp), expectedTy)
    = (case forceTy expectedTy of
           ...
           in T.RecordExtExp { sourceSpan = span, fields = fields, baseExp = baseExp, baseTy = baseTy }
           end
         | expectedTy =>
           let val (actualTy, exp) = synthTypeOfExp (ctx, env, exp)
               val () = checkSubsumption (ctx, env, span, actualTy, expectedTy)
  ...
  | checkTypeOfExp (ctx, env, S.LetInExp (span, decs, innerExp), expectedTy)
    = let val (env', decs) = typeCheckDecs (ctx, env, decs)
          val innerExp = checkTypeOfExp (ctx, mergeEnv (env, env'), innerExp, expectedTy)
      in T.LetInExp (span, decs, innerExp)
      end
  | checkTypeOfExp (ctx, env, S.HandleExp (span, exp, matches), expectedTy)
    (* exp : t, matches : exn -> t *)
    = let val exp = checkTypeOfExp (ctx, env, exp, expectedTy)
          val matches = checkTypeOfMatch (ctx, env, matches, primTy_exn, expectedTy)
      in T.HandleExp (span, exp, matches, expectedTy)
      end
  | checkTypeOfExp (ctx, env, S.RaiseExp (span, exp), expectedTy)
    = let val exp = checkTypeOfExp (ctx, env, exp, primTy_exn)
      in T.RaiseExp (span, expectedTy, exp)
      end
  | checkTypeOfExp (ctx, env, S.IfThenElseExp (span, cond, thenPart, elsePart), expectedTy)
    = let val cond = checkTypeOfExp (ctx, env, cond, primTy_bool)
      ...
      in T.IfThenElseExp (span, cond, thenPart, elsePart)
      end
  | checkTypeOfExp (ctx, env, S.CaseExp (span, exp, matches), expectedTy)
    = let val (expTy, exp) = synthTypeOfExp (ctx, env, exp)
          val matches = checkTypeOfMatch (ctx, env, matches, expTy, expectedTy)
      in T.CaseExp { sourceSpan = span, subjectExp = exp, subjectTy = expTy, matches = matches, matchType = T.CASE, resultTy = expectedTy }
      end
  | checkTypeOfExp (ctx, env, exp as S.FnExp (span, matches), expectedTy)
    = (case forceTy expectedTy of
           T.FnType (_, argTy, retTy) =>
           let val matches = checkTypeOfMatch (ctx, env, matches, argTy, retTy)
           in case matches of
                  [(T.VarPat (_, vid, _), body)] => T.FnExp (span, vid, argTy, body)
                | _ => let val vid = newVId (#context ctx, Syntax.MkVId "a")
                       in T.FnExp (span, vid, argTy, T.CaseExp { sourceSpan = span, subjectExp = T.VarExp (span, T.MkShortVId vid, Syntax.ValueVariable, []), subjectTy = argTy, matches = matches, matchType = T.CASE, resultTy = retTy })
  ...
  | checkTypeOfExp (ctx, env, exp as S.ProjectionExp (span, label), expectedTy)
    = (case forceTy expectedTy of
  ...
  | checkTypeOfExp (ctx, env, exp as S.ListExp (span, xs), expectedTy)
    = (case forceTy expectedTy of
           ...
           if T.eqTyName (con, primTyName_list) then
               let val xs = Vector.map (fn exp => checkTypeOfExp (ctx, env, exp, elemTy)) xs
               in T.ListExp (span, xs, elemTy)
               end
           else
               let val (actualTy, exp) = synthTypeOfExp (ctx, env, exp)
                   val () = checkSubsumption (ctx, env, span, actualTy, expectedTy)
...
and typeCheckDec (ctx : InferenceContext, env : Env, S.ValDec (span, tyvarseq, descs, valbinds))
          ...
          fun generalize ({ sourceSpan = span, pat, exp, expTy, valEnv : (T.VId * T.Ty) S.VIdMap.map, generalizable = false }, (valbinds, valEnvRest : (T.VId * T.TypeScheme) S.VIdMap.map))
              = let val vars = Syntax.VIdMap.listItems valEnv
                in List.app (fn (_, ty) => let val fv = T.freeAnonymousTyVarsInTy ty
                                           in List.app (fn tv as ref (T.Unbound (ct, level)) =>
                 ...
                 ; case vars of
                       [(vid, ty)] => let val valbind' = T.PolyVarBind ( span
                                      ...
                                      in (valbind' :: valbinds, S.VIdMap.unionWith (fn (_, y) => (emitTypeError (ctx, [span], "duplicate identifier in a binding"); y)) (S.VIdMap.map (fn (vid, ty) => (vid, T.TypeScheme ([], ty))) valEnv, valEnvRest))
                                      end
                     | _ => let val espan = T.getSourceSpanOfExp exp
              ...
              = let fun doVal (vid,ty)
                        = let val ty = T.forceTy ty
                              ...
                              val aTyVars = List.foldl (fn (tv, vars) =>
                                                           case !tv of
                                                               T.Unbound (ct as { sourceSpan = _, equalityRequired, class }, level) =>
                                                               if level > #level ctx andalso not (Option.isSome class) then
                ...
                in (valbind' @ valbinds, Syntax.VIdMap.unionWith (fn (_, y) => (emitTypeError (ctx, [span], "duplicate identifier in a binding"); y)) (valEnv', valEnvRest))
                end
  ...
  | typeCheckDec (ctx, env, S.RecValDec (span, tyvarseq, descs, valbinds))
    = let val ctx' = enterLevel ctx
          val valbinds' : (SourcePos.span * (T.Ty * (T.VId * T.Ty) S.VIdMap.map * T.Pat) * S.Exp) list
              = List.map (fn S.PatBind (span, pat, exp) => (span, synthTypeOfPat (ctx', env, pat), exp)) valbinds
          ...
          val valbinds'' = List.map (fn (span, (ty, newValEnv, pat), exp) =>
                                        let val exp = checkTypeOfExp (ctx', localEnv, exp, ty)
                                            val generalizable = isExhaustive (ctx', env, pat) andalso isNonexpansive (env, exp)
                                        in if generalizable then
                                               ()
                                           else
                                               emitTypeError (ctx', [span], "'val rec' must be generalizable")
                                         ; { sourceSpan = span, pat = pat, exp = exp, expTy = ty, valEnv = newValEnv }
                                        end
                                    ) valbinds'
          val tyVars_env = freeTyVarsInEnv (T.TyVarSet.empty, env)
          fun generalize ({ sourceSpan = span, pat = _, exp, expTy, valEnv }, (valbinds, valEnvRest, tyVarsAcc))
              = let fun doVal (vid, ty)
                        = let val ty = T.forceTy ty
                              val tyVars = T.freeTyVarsInTy (tyVars_env, ty)
                              val aTyVars_ty = T.freeAnonymousTyVarsInTy ty
          ...
          fun fixRecursion (span, vid, tysc as T.TypeScheme (tyvars, _), exp)
              = let val unboundTyVars = T.TyVarSet.foldl (fn (tv, acc) =>
                                                             if List.exists (fn (tv', _) => tv = tv') tyvars then
                                                                 ...
                                                                 T.TyVarMap.insert (acc, tv, T.AnonymousTyVar (span, freshTyVar (ctx, span, T.tyVarAdmitsEquality tv, NONE)))
                    ...
                    val subst = List.foldl (fn ((_, vid', T.TypeScheme (tyvars', _), _), subst) =>
                                               T.VIdMap.insert (subst, vid', fn (span, idstatus as Syntax.ValueVariable, []) =>
                                                                           ...
                                                                           | (_, _, _) => emitFatalTypeError (ctx, [span], "invalid use of recursive identifier"))
  ...
  | typeCheckDec(ctx, env, S.DatatypeDec(span, datbinds, typbinds))
    = let val datbinds = let val goConBind = doWithtype (#context ctx, env, typbinds)
          ...
          val datbinds = List.map (fn datbind as S.DatBind (_, _, tycon, _, _) => (datbind, newTyName (#context ctx, tycon))) datbinds
          val partialEnv = envWithTyConEnv (List.foldl (fn ((S.DatBind (span, tyvars, tycon, _, _), tycon'), (m, m')) =>
          ...
          val (tyConMap, tyNameMap, valMap, datbinds)
              = let fun doDatBind ((S.DatBind (span, tyvars, tycon, _, conbinds), tyname), (tyConMap, tyNameMap, accValEnv, datbinds))
                        = let val tyvars = List.map (fn tv => (tv, genTyVar (#context ctx, tv))) tyvars
                              ...
                              val constructorsWithPayload = List.foldl (fn (Syntax.ConBind (_, vid, optTy), set) => if Option.isSome optTy then Syntax.VIdSet.add (set, vid) else set) Syntax.VIdSet.empty conbinds
                              val representation = case conbinds of
                                                       [S.ConBind (_, _, SOME _)] => Syntax.REP_ALIAS
                                                     | [S.ConBind (_, _, NONE)] => Syntax.REP_UNIT
                                                     | _ => if Syntax.VIdSet.isEmpty constructorsWithPayload then
                                                                Syntax.REP_ENUM
                                                            else
                                                                Syntax.REP_BOXED
                              val (valEnv, conbinds) = List.foldr (fn (S.ConBind(span, vid, optTy), (valEnv, conbinds)) =>
                                                                      let val vid' = newVId (#context ctx, vid)
                                                                          ...
                                                                          val tysc = T.TypeScheme (List.map (fn tv => (tv, NONE)) tyvars, case optTy of
    ...
    = let val tystr = lookupTyConInEnv (#context ctx, env, span, longtycon)
          val getLongVId = case longtycon of
                               ...
                               (fn vid => case Syntax.VIdMap.find(#valMap env, vid) of
                                            ...
                                            | NONE => emitFatalTypeError (ctx, [span], "datatype replication: value identifier " ^ Syntax.print_VId vid ^ " not found (internal error)")
                               )
                             | Syntax.MkQualified(strid0 :: strids, _) =>
                               case Syntax.StrIdMap.find(#strMap env, strid0) of
                                   SOME (_, T.MkLongStrId (strid0, strids0)) => (fn vid => T.MkLongVId (strid0, strids0 @ strids, vid))
                                 | NONE => emitFatalTypeError (ctx, [span], "datatype replication: structure " ^ Syntax.print_StrId strid0 ^ " not found (internal error)")
              ...
              = let val vid' = newVId (#context ctx, vid)
                in case lookupLongVIdInEnv(ctx, env, span, longvid) of
                       ...
                       let val optTy = case tysc of
                                         ...
                                         | T.TypeScheme ([], _) => NONE
                                         | T.TypeScheme (_ :: _, _) => (emitTypeError (ctx, [span], "exception constructor must have monomorphic type"); NONE)
                       in (S.VIdMap.insert (valMap, vid, (tysc, ids, T.MkShortVId vid')), T.ExReplication (span, vid', longvid, optTy) :: exbinds)
                       end
                     | Found _ => emitFatalTypeError (ctx, [span], "exception replication: RHS must be an exception constructor")
                     | ValueNotFound notfound => emitFatalTypeError (ctx, [span], "exception replication: RHS must be an exception constructor, but value name '" ^ Syntax.print_LongVId notfound ^ "' was not found")
                     | StructureNotFound notfound => emitFatalTypeError (ctx, [span], "exception replication: RHS must be an exception constructor, but structure name '" ^ Syntax.print_LongStrId notfound ^ "' was not found")
                end
  ...
  | typeCheckDec(ctx, env, S.OpenDec(span, longstrids))
    = let fun getStructure(Syntax.MkQualified([], strid))
              = (case Syntax.StrIdMap.find(#strMap env, strid) of
                   ...
                   | NONE => emitFatalTypeError (ctx, [span], "structure not found")
                )
    ...
    = let val { typeFunction = T.TypeFunction (tyvars, ty), ... } = lookupTyConInEnv (#context ctx, env, span, longtycon)
          val tyname = if List.null tyvars then
                           case ty of
                             ...
                             | _ => emitFatalTypeError (ctx, [span], "overload declaration: longtycon must refer to a concrete type")
                       else
                           emitFatalTypeError (ctx, [span], "overload declaration: longtycon must refer to a concrete type")
  ...
  | typeCheckDec (ctx, env, S.EqualityDec (span, typarams, longtycon, exp))
    = let val { typeFunction = T.TypeFunction (tyvars, ty), ... } = lookupTyConInEnv (#context ctx, env, span, longtycon)
          val tyname = if List.length tyvars = List.length typarams then
                           case ty of
                               T.TyCon (_, tyvars', tyname) => if List.length tyvars' = List.length typarams then
                                                                   ...
                                                                   emitFatalTypeError (ctx, [span], "equality declaration: longtycon must refer to a concrete type")
                             | _ => emitFatalTypeError (ctx, [span], "equality declaration: longtycon must refer to a concrete type")
                       else
                           emitFatalTypeError (ctx, [span], "equality declaration: number of type parameters mismatch")
          ...
          val exp = checkTypeOfExp (ctx, innerEnv, exp, T.FnType (span, T.PairType (span, ty, ty), primTy_bool)) (* allow recursion *)
...
and synthTypeOfMatch (ctx, env, _, (pat0, exp0) :: rest) : T.Ty * T.Ty * (T.Pat * T.Exp) list
...
and checkAndSynthTypeOfMatch (ctx, env, _, (pat0, exp0) :: rest, expectedPatTy) : T.Ty * (T.Pat * T.Exp) list
    = let fun doBranch (pat, exp)
...
and checkTypeOfMatch (ctx, env, patsAndExps : (S.Pat * S.Exp) list, expectedPatTy : T.Ty, expectedExpTy : T.Ty) : (T.Pat * T.Exp) list
    = let fun doBranch (pat, exp)
              = let val (vars, pat') = checkTypeOfPat (ctx, env, pat, expectedPatTy)
                    val env' = mergeEnv (env, envWithValEnv (Syntax.VIdMap.map (fn (vid, ty) => (T.TypeScheme ([], ty), Syntax.ValueVariable, T.MkShortVId vid)) vars))
                in (pat', checkTypeOfExp (ctx, env', exp, expectedExpTy))
                end
      in List.map doBranch patsAndExps
      end
        ...
        fun doTyVar tv = case !tv of
                           ...
                           | T.Unbound ({ sourceSpan, equalityRequired = _, class }, _) =>
                             let val ty = case class of
                                            ...
                                            | SOME (T.Record _) => (emitError (ctx, [sourceSpan], "unresolved flex record"); primTy_unit)
                                            | NONE => primTy_unit
                             in tv := T.Link ty
                             end
...
local
    fun checkValDesc (ctx, _, span, expected as T.TypeScheme (_, _), actual as T.TypeScheme (_, _), origin)
        = let val ictx = { context = ctx
              ...
              fun equalTy (expected as T.TyVar (_, tv), T.TyVar (_, tv'))
                  = if tv = tv' then
                        ...
                        onMismatch expected
                | equalTy (expected as T.AnonymousTyVar (_, data as ref (T.Unbound ({ equalityRequired = isEqType, ... }, _))), T.AnonymousTyVar (_, data' as ref (T.Unbound ({ equalityRequired = isEqType', ... }, _))))
                  = if data <> data' then
    ...
    fun checkExp (_ : Context, _, T.SConExp _) : unit = ()
      ...
      | checkExp (ctx, env, T.RecordExp (_, fields)) = List.app (fn (_, exp) => checkExp (ctx, env, exp)) fields
      | checkExp (ctx, env, T.RecordExtExp { sourceSpan = _, fields, baseExp, baseTy = _ })
        = ( List.app (fn (_, exp) => checkExp (ctx, env, exp)) fields
          ; checkExp (ctx, env, baseExp)
          )
      | checkExp (ctx, env, T.LetInExp (_, decs, exp)) = (checkDecs (ctx, env, decs); checkExp (ctx, env, exp))
      | checkExp (ctx, env, T.AppExp (_, e1, e2)) = (checkExp (ctx, env, e1); checkExp (ctx, env, e2))
      | checkExp (ctx, env, T.TypedExp (_, e, _)) = checkExp (ctx, env, e)
      | checkExp (ctx, env, T.HandleExp (_, exp, match, _)) = (checkExp (ctx, env, exp); checkMatch (ctx, env, match))
      | checkExp (ctx, env, T.RaiseExp (_, _, exp)) = checkExp (ctx, env, exp)
      | checkExp (ctx, env, T.IfThenElseExp (_, e1, e2, e3)) = (checkExp (ctx, env, e1); checkExp (ctx, env, e2); checkExp (ctx, env, e3))
      | checkExp (ctx, env, T.CaseExp { sourceSpan = _, subjectExp, subjectTy = _, matches, matchType = _, resultTy = _ }) = (checkExp (ctx, env, subjectExp); checkMatch (ctx, env, matches))
      | checkExp (ctx, env, T.FnExp (_, _, _, exp)) = checkExp (ctx, env, exp)
      | checkExp (_, _, T.ProjectionExp _) = ()
      | checkExp (ctx, env, T.ListExp (_, elems, _)) = Vector.app (fn e => checkExp (ctx, env, e)) elems
      | checkExp (ctx, env, T.VectorExp (_, elems, _)) = Vector.app (fn e => checkExp (ctx, env, e)) elems
      | checkExp (ctx, env, T.PrimExp (_, _, _, args)) = Vector.app (fn e => checkExp (ctx, env, e)) args
      | checkExp (_, _, T.BogusExp _) = ()
    and checkDec (ctx, env, T.ValDec (_, valbinds)) = List.app (fn valbind => checkValBind (ctx, env, valbind)) valbinds
      | checkDec (ctx, env, T.RecValDec (_, valbinds)) = List.app (fn valbind => checkValBind (ctx, env, valbind)) valbinds
      | checkDec (ctx, env, T.IgnoreDec (_, exp, _)) = checkExp (ctx, env, exp)
      | checkDec (_, _, T.TypeDec _) = ()
      | checkDec (_, _, T.DatatypeDec _) = ()
      | checkDec (_, _, T.ExceptionDec _) = ()
      | checkDec (ctx, env, T.OverloadDec (_, _, _, map)) = Syntax.OverloadKeyMap.app (fn exp => checkExp (ctx, env, exp)) map
      | checkDec (ctx, env, T.EqualityDec (_, tyvars, _, exp)) = checkExp (ctx, T.TyVarSet.addList (env, tyvars), exp)
    ...
    and checkMatch (ctx, env, matches) = List.app (fn (_, exp) => checkExp (ctx, env, exp)) matches
    and checkValBind (ctx, env, T.TupleBind (_, _, exp)) = checkExp (ctx, env, exp)
      | checkValBind (ctx, env, T.PolyVarBind (_, _, T.TypeScheme (tyvars, _), exp)) = checkExp (ctx, T.TyVarSet.addList (env, List.map #1 tyvars), exp)
in
...
fun checkTyScope (ctx, tvset : T.TyVarSet.set, tynameset : T.TyNameSet.set)
    = let fun goTy (T.TyVar (span, tv))
            ...
            | goTy (T.RecordExtType (_, fields, baseTy)) = ( Syntax.LabelMap.app goTy fields; goTy baseTy )
            ...
            | goTy (T.FnType (_, ty1, ty2)) = ( goTy ty1; goTy ty2 )
          ...
          fun goPat (T.WildcardPat _) = ()
            ...
            | goPat (T.RecordPat { sourceSpan = _, fields, ellipsis, wholeRecordType })
              = ( List.app (fn (_, pat) => goPat pat) fields
                ; Option.app goPat ellipsis
                ; goTy wholeRecordType
                )
            | goPat (T.ConPat { sourceSpan = _, longvid = _, payload, tyargs, valueConstructorInfo = _ }) = ( List.app goTy tyargs
                                                                                                            ; Option.app (fn (ty, pat) => (goTy ty; goPat pat)) payload
                                                                                                            )
            | goPat (T.TypedPat (_, pat, ty)) = ( goTy ty; goPat pat )
            | goPat (T.LayeredPat (_, _, ty, pat)) = ( goTy ty; goPat pat )
            | goPat (T.VectorPat (_, pats, _, elemTy)) = ( goTy elemTy; Vector.app goPat pats )
          fun goExp (T.SConExp (_, _, ty)) = goTy ty
            | goExp (T.VarExp (_, _, _, tyargs)) = List.app (fn (ty, _) => goTy ty) tyargs
            | goExp (T.RecordExp (_, fields)) = List.app (fn (_, exp) => goExp exp) fields
            | goExp (T.RecordExtExp { sourceSpan = _, fields, baseExp, baseTy })
              = ( List.app (fn (_, exp) => goExp exp) fields
                ; goExp baseExp
                ; goTy baseTy
                )
            | goExp (T.LetInExp (_, decs, exp)) = let val tynameset = goDecs decs
                                                      val { goExp, ... } = checkTyScope (ctx, tvset, tynameset)
                                                  in goExp exp
                                                  end
            | goExp (T.AppExp (_, exp1, exp2)) = ( goExp exp1; goExp exp2 )
            | goExp (T.TypedExp (_, exp, ty)) = ( goExp exp; goTy ty )
            | goExp (T.HandleExp (_, exp, matches, resultTy)) = ( goExp exp; goTy resultTy; List.app (fn (pat, exp) => (goPat pat; goExp exp)) matches )
            | goExp (T.RaiseExp (_, ty, exp)) = ( goTy ty; goExp exp )
            | goExp (T.IfThenElseExp (_, exp1, exp2, exp3)) = ( goExp exp1; goExp exp2; goExp exp3 )
            ...
            | goExp (T.FnExp (_, _, ty, exp)) = ( goTy ty; goExp exp )
            | goExp (T.ProjectionExp { sourceSpan = _, label = _, recordTy, fieldTy }) = ( goTy recordTy; goTy fieldTy )
            | goExp (T.ListExp (_, xs, ty)) = ( Vector.app goExp xs ; goTy ty )
            | goExp (T.VectorExp (_, xs, ty)) = ( Vector.app goExp xs ; goTy ty )
            | goExp (T.PrimExp (_, _, tyargs, args)) = ( Vector.app goTy tyargs ; Vector.app goExp args )
            | goExp (T.BogusExp (_, ty)) = goTy ty
          and goDec (T.ValDec (_, valbinds)) = ( List.app goValBind valbinds
                                               ; tynameset
                                               )
            | goDec (T.RecValDec (_, valbinds)) = ( List.app goValBind valbinds
                                                  ; tynameset
                                                  )
            | goDec (T.IgnoreDec (_, exp, ty)) = ( goExp exp ; goTy ty ; tynameset )
            | goDec (T.TypeDec (_, typbinds)) = let fun goTypBind (T.TypBind (_, tyvars, _, ty)) = let val { goTy, ... } = checkTyScope (ctx, T.TyVarSet.addList (tvset, tyvars), tynameset)
                                                                                                   in goTy ty
                                                                                                   end
                                                in List.app goTypBind typbinds
                                                 ; tynameset
                                                end
            | goDec (T.DatatypeDec (_, datbinds)) = let val tynameset = List.foldl (fn (T.DatBind (_, _, tyname, _, _), tynameset) => T.TyNameSet.add (tynameset, tyname)) tynameset datbinds
                                                        fun goDatBind (T.DatBind (_, tyvars, _, conbinds, _))
                                                            = let val { goTy, ... } = checkTyScope (ctx, T.TyVarSet.addList (tvset, tyvars), tynameset)
                                                                  fun goConBind (T.ConBind (_, _, optTy, _)) = Option.app goTy optTy
                                                              in List.app goConBind conbinds
                                                              end
                                                    in List.app goDatBind datbinds
                                                     ; tynameset
                                                    end
            | goDec (T.ExceptionDec (_, exbinds)) = ( List.app (fn T.ExBind (_, _, optTy) => Option.app goTy optTy
                                                               | T.ExReplication (_, _, _, optTy) => Option.app goTy optTy
                                                               ) exbinds
                                                    ; tynameset
                                                    )
            | goDec (T.OverloadDec (span, _, tyname, map)) = ( if T.TyNameSet.member (tynameset, tyname) then
                                                                   ()
                                                               else
                                                                   emitError (ctx, [span], "type constructor scope violation: " ^ TypedSyntax.PrettyPrint.print_TyName tyname)
            ...
            | goDec (T.ValDescDec { sourceSpan = _, expected = T.TypeScheme (tyvars, ty), actual = T.TypeScheme (tyvars', ty'), origin = _ })
              = ( #goTy (checkTyScope (ctx, T.TyVarSet.addList (tvset, List.map #1 tyvars), tynameset)) ty
                ; #goTy (checkTyScope (ctx, T.TyVarSet.addList (tvset, List.map #1 tyvars'), tynameset)) ty'
                ; tynameset
                )
            | goDec (T.ESImportDec { sourceSpan = _, pure = _, specs, moduleName = _ })
          ...
          and goDecs decs = List.foldl (fn (dec, tynameset) => let val { goDec, ... } = checkTyScope (ctx, tvset, tynameset)
                                       ...
                                       tynameset decs
          and goValBind (T.TupleBind (_, binds, exp)) = ( List.app (fn (_, ty) => goTy ty) binds
                                                        ; goExp exp
                                                        )
          ...
          fun goStrExp (T.StructExp _) = tynameset
            ...
            | goStrExp (T.PackedStrExp { sourceSpan = _, strExp, payloadTypes = _, packageSig }) = let val _ = goStrExp strExp : T.TyNameSet.set
                                                                                                   in List.foldl (fn ({ tyname, ... }, set) => T.TyNameSet.add (set, tyname)) tynameset (#bound packageSig)
                                                                                                   end
            | goStrExp (T.FunctorAppExp { sourceSpan = _, funId = _, argumentTypes = _, argumentStr, packageSig })
              = let val tynameset = goStrExp argumentStr
                    (* TODO: Check argumentTypes *)
                in List.foldl (fn ({ tyname, ... }, set) => T.TyNameSet.add (set, tyname)) tynameset (#bound packageSig)
                end
            | goStrExp (T.LetInStrExp (_, strdecs, strexp)) = let val tynameset = goStrDecs strdecs
                                                                  val { goStrExp, ... } = checkTyScope (ctx, tvset, tynameset)
                                                              in goStrExp strexp
                                                              end
          and goStrDec (T.CoreDec (_, dec)) = goDec dec
            | goStrDec (T.StrBindDec (_, _, strexp, { s = _, bound })) = List.foldl (fn ({ tyname, ... }, set) => T.TyNameSet.add (set, tyname)) (goStrExp strexp) bound
          and goStrDecs decs = List.foldl (fn (dec, tynameset) => let val { goStrDec, ... } = checkTyScope (ctx, tvset, tynameset)
                                                                  in goStrDec dec
                                                                  end)
                                          tynameset decs
          fun goFunExp (tynames, _, _, strexp) = let val tynameset' = List.foldl (fn ({ tyname, ...}, set) => T.TyNameSet.add (set, tyname)) tynameset tynames
                                                     val { goStrExp, ... } = checkTyScope (ctx, tvset, tynameset')
...
fun canonicalOrderForQSignature ({ s, bound } : T.QSignature) : T.TyName list
    = let val bound' = T.TyNameMap.mapi (fn (tyname, { arity = _, admitsEquality = _, longtycon }) => Option.getOpt (canonicalPathForTyName (s, tyname), longtycon)) bound
...
and canonicalPathForTyName ({ valMap = _, tyConMap, strMap } : T.Signature, tyname : T.TyName) : Syntax.LongTyCon option
    = let val t = Syntax.TyConMap.filter (fn { typeFunction = T.TypeFunction (tyvars, ty), valEnv = _ } =>
                                             case ty of
            ...
            | goTy (T.TyCon (span, tyargs, tycon)) = (case T.TyNameMap.find (subst, tycon) of
                                                        ...
                                                        | SOME (T.TypeFunction (tyvars, ty)) =>
                                                          let val subst' = (ListPair.foldlEq (fn (tv, tyarg, m) => TypedSyntax.TyVarMap.insert (m, tv, goTy tyarg)) TypedSyntax.TyVarMap.empty (tyvars, tyargs))
                                                                           handle ListPair.UnequalLengths => emitFatalError (ctx, [span], "invalid type constructor substitution")
                                                          in T.applySubstTy subst' ty
                                                          end
                                                     )
...
fun refreshTyNameInTy (_ : Context, subst : T.TyName T.TyNameMap.map) : T.Ty -> T.Ty
    = let fun goTy (ty as T.TyVar _) = ty
            | goTy (ty as T.AnonymousTyVar _) = ty
            | goTy (T.RecordType (span, fields)) = T.RecordType (span, Syntax.LabelMap.map goTy fields)
            | goTy (T.RecordExtType (span, fields, baseTy)) = T.RecordExtType (span, Syntax.LabelMap.map goTy fields, goTy baseTy)
...
fun checkEquality (ctx : Context, env : ('val,'str) Env', tyvars : T.TyVarSet.set) : T.Ty -> bool
    = let fun goTy (T.TyVar (_, tv)) = if T.TyVarSet.member (tyvars, tv) then
                                           true
                                       else
            ...
            | goTy (T.RecordExtType (_, fields, baseTy)) = Syntax.LabelMap.all goTy fields andalso goTy baseTy
...
fun getTypeNameFromTypeStructure (_ (* ctx *), { typeFunction = T.TypeFunction (tyvars, T.TyCon (_, tyargs, tyname)), ... } : T.TypeStructure) : (T.TyName * int) option
...
fun evalSignature (ctx : Context, env : SigEnv, S.BasicSigExp (_, specs)) : T.QSignature
      ...
      in case getTypeNameFromTypeStructure(ctx, tystr) of
             ...
             if List.length tyvars = arity then
                 case T.TyNameMap.find (#bound s, tyname) of
                   ...
                   | NONE => emitFatalError (ctx, [span], "type realisation against a rigid type")
             else
                 emitFatalError (ctx, [span], "type realisation against a rigid type")
           | NONE => emitFatalError (ctx, [span], "type realisation against a rigid type")
      end
and evalSpecs (ctx : Context, env : SigEnv, specs) : T.QSignature
    = List.foldl (fn (spec, s) => let val env' = addSignatureToEnv (env, #s s, T.TyNameMap.map (fn { arity, admitsEquality, longtycon = _ } => { arity = arity, admitsEquality = admitsEquality, overloadClass = NONE }) (#bound s))
...
and addSpec (ctx : Context, env : SigEnv, S.ValDesc (_, descs)) : T.QSignature
    = { s = { valMap = List.foldl (fn ((vid, ty), valMap) => let val tvs = PostParsing.freeTyVarsInTy(Syntax.TyVarSet.empty, ty)
                                                             ...
                                                             in Syntax.VIdMap.insert (valMap, vid, (T.TypeScheme (Syntax.TyVarMap.foldr (fn (tv, xs) => (tv, NONE) :: xs) [] tvs, ty), Syntax.ValueVariable))
  ...
  | addSpec (ctx, env, S.DatDesc (span, descs : (S.TyVar list * S.TyCon * S.optional_bar * S.ConBind list) list, typbinds))
    = let val descs = let val goConBind = doWithtype(ctx, env, typbinds)
          ...
          val withtypeMap = List.foldl (fn (S.TypBind (_, tyvars, tycon, ty), tyConMap) =>
      ...
      in List.foldl (fn ((tycon, tyname, tyvarPairs, tystr, condescs), s) =>
                        let val { typeFunction as T.TypeFunction (tyvars, ty), ... } = tystr
                            ...
                            val allConstructors = List.foldl (fn (Syntax.ConBind (_, vid, _), set) => Syntax.VIdSet.add (set, vid)) Syntax.VIdSet.empty condescs
                            val constructorsWithPayload = List.foldl (fn (Syntax.ConBind (_, vid, optTy), set) => if Option.isSome optTy then Syntax.VIdSet.add (set, vid) else set) Syntax.VIdSet.empty condescs
                            val representation = case condescs of
                                                     [S.ConBind (_, _, SOME _)] => Syntax.REP_ALIAS
                                                   | [S.ConBind (_, _, NONE)] => Syntax.REP_UNIT
                                                   | _ => if Syntax.VIdSet.isEmpty constructorsWithPayload then
                                                              Syntax.REP_ENUM
                            ...
                            val valEnv = List.foldl (fn (S.ConBind(span, vid, optTy), valEnv) =>
                                                        let val tysc = T.TypeScheme (List.map (fn tv => (tv, NONE)) tyvars, case optTy of
                                                                                                                                NONE => ty
  ...
  | addSpec (ctx, env, S.StrDesc (_, descs)) = let val strMap = List.foldl (fn ((strid, sigexp), m) => Syntax.StrIdMap.insert (m, strid, evalSignature (ctx, env, sigexp))) Syntax.StrIdMap.empty descs
                                               in { s = { valMap = Syntax.VIdMap.empty
                                                        , tyConMap = Syntax.TyConMap.empty
                                                        , strMap = Syntax.StrIdMap.map (fn { s, bound = _ } => T.MkSignature s) strMap
                                                        }
  ...
  | addSpec (ctx, env, S.Include (_, sigexp)) = evalSignature (ctx, env, sigexp)
  ...
  | addSpec (ctx, _, S.Sharing (span, _, [])) = emitFatalError (ctx, [span], "sharing: empty longtycons (internal error)")
                      ...
                      let val typeFn = let val tyvars = List.tabulate(arity, fn _ => genTyVar(ctx, Syntax.MkTyVar "a"))
                          ...
                          val (subst, admitsEquality) = List.foldl (fn (tystr, (subst, admitsEquality)) =>
                                                                       case getTypeNameFromTypeStructure(ctx, tystr) of
                                                                           SOME (tyname, arity') =>
                                                                           if arity' <> arity then
                                                                               emitFatalError (ctx, [span], "sharing: arity mismatch")
                                                                           else
                                                                               (case T.TyNameMap.find (#bound s, tyname) of
...
fun sameType (T.TyVar (_, tv), T.TyVar (_, tv')) = tv = tv'
  | sameType (T.RecordType (_, fields), T.RecordType (_, fields')) = Syntax.LabelMap.numItems fields = Syntax.LabelMap.numItems fields'
                                                                     andalso Syntax.LabelMap.alli (fn (label, ty) => case Syntax.LabelMap.find (fields', label) of
                                                                                                                         SOME ty' => sameType (ty, ty')
                                                                                                                       | NONE => false
                                                                                                  ) fields
  | sameType (T.TyCon (_, tyargs, tycon), T.TyCon (_, tyargs', tycon')) = T.eqTyName (tycon, tycon') andalso (ListPair.allEq sameType (tyargs, tyargs') handle ListPair.UnequalLengths => false)
  | sameType (T.FnType (_, ty1, ty2), T.FnType (_, ty1', ty2')) = sameType (ty1, ty1') andalso sameType (ty2, ty2')
  | sameType (_, _) = false
                                                            ...
                                                            let val { typeFunction as T.TypeFunction (tyvars, actualTy), ... }
                                                                    = let val s = lookupStr (ctx, actual, span, strids)
                                                                      in case Syntax.TyConMap.find (#tyConMap s, tycon) of
                                                                             SOME tystr => tystr
                                                                           | NONE => emitFatalError (ctx, [span], "signature matching: type not found: " ^ Syntax.print_LongTyCon longtycon)
                                                                      end
    ...
    = let val strMap : (T.Signature * T.StrExp) S.StrIdMap.map
              = Syntax.StrIdMap.mapi (fn (strid, T.MkSignature s) =>
                                         case Syntax.StrIdMap.find(#strMap actual, strid) of
                                           ...
                                           | NONE => emitFatalError (ctx, [span], "signature matching: structure not found (" ^ Syntax.print_StrId strid ^ ")")
                                     ) (#strMap expected)
          val tyConMap : T.TypeStructure S.TyConMap.map
              = Syntax.TyConMap.mapi (fn (tycon, expectedTyStr) =>
                                         case Syntax.TyConMap.find (#tyConMap actual, tycon) of
                                           ...
                                           | NONE => emitFatalError (ctx, [span], "signature matching: " ^ (case tycon of Syntax.MkTyCon name => name) ^ " not found")
                                     ) (#tyConMap expected)
              ...
              = Syntax.VIdMap.mapi (fn (vid, (tyscE, idsE)) =>
                                       case Syntax.VIdMap.find (#valMap actual, vid) of
                                           SOME (tyscA, idsA) => let val longvid = case longstrid of
                                                                     ...
                                                                     val innerContext = { nextTyVar = #nextTyVar ctx
                                                                                        , nextVId = #nextVId ctx
                                         ...
                                         | NONE => emitFatalError(ctx, [span], "signature matching: " ^ Syntax.getVIdName vid ^ " not found")
                                   ) (#valMap expected)
          ...
          val (decs, valMap) = Syntax.VIdMap.foldli (fn (vid, (_, decs, longvid, ids), (decs', valMap)) =>
...
and matchTyDesc (ctx, _, span, expected : T.TypeStructure, actual : T.TypeStructure) : T.TypeStructure
...
and matchValDesc (ctx, env, span, expected : T.TypeScheme, longvid : T.LongVId, actual : T.TypeScheme, ids : Syntax.ValueConstructorInfo Syntax.IdStatus) : T.TypeScheme * T.Dec list * T.LongVId
    = let val T.TypeScheme (tyvarsE, tyE) = expected
          ...
          val trivial = ListPair.allEq (fn ((tvE, NONE), (T.TyVar (_, tvA), NONE)) => T.eqUTyVar (tvE, tvA)
                                       | ((tvE, SOME T.IsEqType), (T.TyVar (_, tvA), SOME T.IsEqType)) => T.eqUTyVar (tvE, tvA)
                                       | _ => false
                                       ) (tyvarsE, tyargsA)
      in if trivial then
...
fun typeCheckStrExp (ctx : Context, env : Env, S.StructExp (span, decs)) : T.PackedSignature * TyNameAttr T.TyNameMap.map * T.StrDec list * T.StrExp
  ...
  | typeCheckStrExp(ctx, env, S.OpaqueConstraintExp(span, strexp, sigexp))
    = let val (sA, tyNameMap, decs, strexp) = typeCheckStrExp(ctx, env, strexp)
          ...
          val strid = newStrId(ctx, Syntax.MkStrId "tmp")
          val (_, strexp') = matchQSignature (ctx, env', span, sE, strid, #s sA)
          val tynames = canonicalOrderForQSignature sE
          val packageSig = { s = #s sE
                           , bound = List.map (fn tyname => let val { arity, admitsEquality, longtycon = _ } = T.TyNameMap.lookup (#bound sE, tyname)
                                                            in { tyname = tyname, arity = arity, admitsEquality = admitsEquality }
                                                            end
                                              ) tynames
                           }
          val tyNameMapOutside = T.TyNameMap.foldli (fn (tyname, { arity, admitsEquality, longtycon = _ }, acc) =>
          ...
          val payloadTypes = List.map (fn tyname => let val { longtycon = Syntax.MkQualified (strids, tycon), ... } = T.TyNameMap.lookup (#bound sE, tyname)
                                                    ...
                                                    in case Syntax.TyConMap.find (tyConMap, tycon) of
                                                         ...
                                                         | NONE => emitFatalError (ctx, [span], "unknown type constructor")
                                                    end) tynames
    ...
    = (case S.FunIdMap.find (#funMap env, funid) of
         ...
         | SOME (funsig, funid) =>
           let val { bound, paramSig, resultSig } = funsig (* TODO: refresh bound tynames? *)
               ...
               val instantiation = List.foldl (fn ((tyname, typeFunction, _), map) => T.TyNameMap.insert (map, tyname, typeFunction)) T.TyNameMap.empty argumentTypes
           ...
           in ( resultSig
              ...
              , T.FunctorAppExp { sourceSpan = span
                                , funId = funid
                                , argumentTypes = List.map (fn (_, typeFunction, admitsEquality) => { typeFunction = typeFunction, admitsEquality = admitsEquality }) argumentTypes
                                , argumentStr = strexp'
  ...
  | typeCheckStrExp (ctx, env, S.LetInStrExp (_, strdecs, strexp)) = let val (env', strdecs) = typeCheckStrDecs (ctx, env, strdecs)
...
fun typeCheckFunExp' (ctx, span, paramEnv, paramSig, strid, strexp) : TypedSyntax.FunSig * TypedSyntax.FunExp
    = let val (actualSignature : T.PackedSignature, bodyTyNameMap, strdecs, strexp) = typeCheckStrExp (ctx, paramEnv, strexp)
          ...
          val payloadTypes = List.map (fn { tyname, arity, admitsEquality = _ } =>
          ...
          val funexp = ( List.map (fn tyname => let val { arity, admitsEquality, longtycon = _ } = T.TyNameMap.lookup (#bound paramSig, tyname)
                                                in { tyname = tyname, arity = arity, admitsEquality = admitsEquality }
                                                end
                                  ) tynamesInParam
                       , strid
...
fun typeCheckFunExp (ctx, span, env, S.NamedFunExp (strid, sigexp, strexp)) : TypedSyntax.FunSig * TypedSyntax.FunExp
    = let val strid' = newStrId(ctx, strid)
          ...
          val tyNameMap : TyNameAttr T.TyNameMap.map
              = T.TyNameMap.map (fn { arity, admitsEquality, longtycon = _ } =>
                                    { arity = arity
                                    , admitsEquality = admitsEquality
                                    , overloadClass = NONE
                                    }
...
(*: val typeCheckProgram : Context * Env * ((Syntax.Dec Syntax.TopDec) list) list -> Env * (TypedSyntax.TopDec list) list *)
fun typeCheckProgram (_, _, [] : ((Syntax.Dec Syntax.TopDec) list) list) : Env * (TypedSyntax.TopDec list) list = (emptyEnv, [])
