...
fun doPat prec (F.WildcardPat _) = [P.Fragment "_"]
  | doPat prec (F.SConPat { scon = F.IntegerConstant x, ... }) = [P.Fragment (IntInf.toString x)]
  | doPat prec (F.SConPat { scon = F.WordConstant x, ... }) = [P.Fragment (IntInf.toString x)]
  | doPat prec (F.SConPat { scon = F.CharConstant x, ... }) = [P.Fragment (Char.toString x)]
  | doPat prec (F.SConPat { scon = F.Char16Constant x, ... }) = [P.Fragment (Int.toString x)]
  | doPat prec (F.SConPat { scon = F.StringConstant x, ... }) = [P.Fragment (String.toString x)]
  | doPat prec (F.SConPat { scon = F.String16Constant x, ... }) = [P.Fragment "<string16>"]
  | doPat prec (F.VarPat (_, vid, ty)) = showParen (prec >= 1) (P.Fragment (TypedSyntax.print_VId vid) :: P.Fragment " : " :: doTy 0 ty)
  | doPat prec (F.RecordPat { sourceSpan, fields, ellipsis, allFields }) = P.Fragment "{" :: P.commaSep (List.foldr (fn ((label, pat), xs) => (doLabel label @ P.Fragment ": " :: doPat 0 pat) :: xs) (case ellipsis of SOME basePat => [P.Fragment "...=" :: doPat 0 basePat] | NONE => []) fields) @ [P.Fragment "}"]
...
and doExp prec (F.PrimExp (primOp, types, exps)) = P.Fragment "_prim." :: doPrimOp primOp @ P.Fragment " [" :: P.commaSep (List.map (doTy 0) types) @ (P.Fragment "] (" :: P.commaSep (List.map (doExp 0) exps) @ [P.Fragment ")"])
  ...
  | doExp prec (F.CaseExp { sourceSpan, subjectExp, subjectTy, matches, matchType, resultTy }) = showParen (prec >= 1) (P.Fragment "case " :: doExp 0 subjectExp @ P.Fragment " : " :: doTy 0 subjectTy @ P.Fragment " of" :: P.LineTerminator :: P.IncreaseIndent 2 :: List.foldr (fn ((pat, exp), rest) => P.Indent :: P.Fragment "| " :: doPat 0 pat @ P.Fragment " => " :: doExp 0 exp @ P.LineTerminator :: rest) [P.DecreaseIndent 2, P.Indent] matches)
