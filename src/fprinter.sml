...
fun doPat prec (F.WildcardPat _) = [P.Fragment "_"]
  ...
  | doPat prec (F.ValConPat (_, path, SOME innerPat, tyargs, info)) = showParen (prec >= 1) (doPath path @ P.Fragment "[" :: P.commaSep (List.map (doTy 0) tyargs) @ P.Fragment "] " :: doPat 1 innerPat)
  | doPat prec (F.ExnConPat (_, path, NONE, tyargs)) = showParen (prec >= 1) (doPath path @ P.Fragment "[" :: P.commaSep (List.map (doTy 0) tyargs) @ [P.Fragment "]"])
  | doPat prec (F.ExnConPat (_, path, SOME innerPat, tyargs)) = showParen (prec >= 1) (doPath path @ P.Fragment "[" :: P.commaSep (List.map (doTy 0) tyargs) @ P.Fragment "] " :: doPat 1 innerPat)
...
and doDec (F.ValDec (vid, SOME ty, exp)) = P.Fragment "val " :: P.Fragment (TypedSyntax.print_VId vid) :: P.Fragment " : " :: doTy 0 ty @ P.Fragment " = " :: doExp 0 exp
  ...
  | doDec (F.ExceptionRepDec { conName, conPath, tagPath, payloadTy = NONE }) = P.Fragment "exception " :: P.Fragment (TypedSyntax.print_VId conName) :: P.Fragment " = " :: doPath conPath @ P.Fragment ", tag=" :: doPath tagPath
  | doDec (F.ExceptionRepDec { conName, conPath, tagPath, payloadTy = SOME payloadTy }) = P.Fragment "exception " :: P.Fragment (TypedSyntax.print_VId conName) :: P.Fragment " of " :: doTy 0 payloadTy @ P.Fragment " = " :: doPath conPath @ P.Fragment ", tag=" :: doPath tagPath
