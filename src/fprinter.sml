...
fun doTy _ (F.TyVar tv) = doTyVar tv
  | doTy _ (F.RecordType fields) = P.Fragment "{" :: P.commaSep (Syntax.LabelMap.foldri (fn (label, ty, xs) => (doLabel label @ P.Fragment ": " :: doTy 0 ty) :: xs) [] fields) @ [P.Fragment "}"]
...
fun doPrimOp (F.IntConstOp x) = [P.Fragment ("int " ^ IntInf.toString x)]
  ...
  | doPrimOp (F.Char8ConstOp x) = [P.Fragment ("char8 \"" ^ Char.toString x ^ "\"")]
  | doPrimOp (F.Char16ConstOp x) = [P.Fragment ("char16 \"" ^ StringElement.charToString (StringElement.CODEUNIT x) ^ "\"")]
  | doPrimOp (F.String8ConstOp x) = [P.Fragment ("string8 \"" ^ String.toString x ^ "\"")]
  | doPrimOp (F.String16ConstOp x) = [P.Fragment ("string16 \"" ^ Vector.foldr (fn (c, acc) => StringElement.charToString (StringElement.CODEUNIT c) ^ acc) "\"" x)]
  ...
  | doPrimOp (F.PrimCall primOp) = [P.Fragment (Primitives.toString primOp)]
...
fun doPat _ (F.WildcardPat _) = [P.Fragment "_"]
  | doPat _ (F.SConPat { scon = F.IntegerConstant x, ... }) = [P.Fragment (IntInf.toString x)]
  | doPat _ (F.SConPat { scon = F.WordConstant x, ... }) = [P.Fragment (IntInf.toString x)]
  | doPat _ (F.SConPat { scon = F.CharConstant x, ... }) = [P.Fragment (Char.toString x)]
  | doPat _ (F.SConPat { scon = F.Char16Constant x, ... }) = [P.Fragment (Int.toString x)]
  | doPat _ (F.SConPat { scon = F.StringConstant x, ... }) = [P.Fragment (String.toString x)]
  | doPat _ (F.SConPat { scon = F.String16Constant _, ... }) = [P.Fragment "<string16>"]
  | doPat prec (F.VarPat (_, vid, ty)) = showParen (prec >= 1) (P.Fragment (TypedSyntax.print_VId vid) :: P.Fragment " : " :: doTy 0 ty)
  | doPat _ (F.RecordPat { sourceSpan = _, fields, ellipsis, allFields = _ }) = P.Fragment "{" :: P.commaSep (List.foldr (fn ((label, pat), xs) => (doLabel label @ P.Fragment ": " :: doPat 0 pat) :: xs) (case ellipsis of SOME basePat => [P.Fragment "...=" :: doPat 0 basePat] | NONE => []) fields) @ [P.Fragment "}"]
  | doPat prec (F.ValConPat { sourceSpan = _, info, payload = NONE }) = showParen (prec >= 1) [P.Fragment (#tag info)]
  | doPat prec (F.ValConPat { sourceSpan = _, info, payload = SOME (_, payloadPat) }) = showParen (prec >= 1) (P.Fragment (#tag info) :: P.Fragment " " :: doPat 1 payloadPat)
  | doPat prec (F.ExnConPat { sourceSpan = _, tagPath, payload = NONE }) = showParen (prec >= 1) (doExp 0 tagPath)
  | doPat prec (F.ExnConPat { sourceSpan = _, tagPath, payload = SOME (_, payloadPat) }) = showParen (prec >= 1) (doExp 0 tagPath @ P.Fragment " " :: doPat 1 payloadPat)
  ...
  | doPat _ (F.VectorPat (_, pats, wildcard, _)) = P.Fragment "#[" :: P.commaSep (Vector.foldr (fn (pat, xs) => doPat 0 pat :: xs) (if wildcard then [[P.Fragment "..."]] else []) pats) @ [P.Fragment "]"] (* elemTy? *)
(* precedence
 ...
 *)
and doExp _ (F.PrimExp (primOp, types, exps)) = P.Fragment "_prim." :: doPrimOp primOp @ P.Fragment " [" :: P.commaSep (List.map (doTy 0) types) @ (P.Fragment "] (" :: P.commaSep (List.map (doExp 0) exps) @ [P.Fragment ")"])
  | doExp _ (F.VarExp vid) = [P.Fragment (TypedSyntax.print_VId vid)]
  | doExp _ (F.RecordExp fields) = P.Fragment "{" :: P.commaSep (List.foldr (fn ((label, exp), xs) => (doLabel label @ P.Fragment " = " :: doExp 0 exp) :: xs) [] fields) @ [P.Fragment "}"]
  ...
  | doExp prec (F.CaseExp { sourceSpan = _, subjectExp, subjectTy, matches, matchType = _, resultTy = _ }) = showParen (prec >= 1) (P.Fragment "case " :: doExp 0 subjectExp @ P.Fragment " : " :: doTy 0 subjectTy @ P.Fragment " of" :: P.LineTerminator :: P.IncreaseIndent 2 :: List.foldr (fn ((pat, exp), rest) => P.Indent :: P.Fragment "| " :: doPat 0 pat @ P.Fragment " => " :: doExp 0 exp @ P.LineTerminator :: rest) [P.DecreaseIndent 2, P.Indent] matches)
  ...
  | doExp prec (F.ProjectionExp { label, record, fieldTypes = _ }) = showParen (prec >= 2) (P.Fragment "#" :: doLabel label @ P.Fragment " " :: doExp 2 record)
  ...
  | doExp _ (F.ExportValue exp) = P.Fragment "_export " :: doExp 2 exp
  | doExp _ (F.ExportModule fields) = P.Fragment "_export {" :: P.commaSepV (Vector.map (fn (name, exp) => P.Fragment name :: P.Fragment " = " :: doExp 0 exp) fields) @ [P.Fragment "}"]
and doDec (F.ValDec (vid, SOME ty, exp)) = P.Fragment "val " :: P.Fragment (TypedSyntax.print_VId vid) :: P.Fragment " : " :: doTy 0 ty @ P.Fragment " = " :: doExp 0 exp
  ...
  | doDec (F.ExceptionDec { name = _, tagName, payloadTy = NONE }) = P.Fragment "exception " :: P.Fragment (TypedSyntax.print_VId tagName) :: []
  | doDec (F.ExceptionDec { name = _, tagName, payloadTy = SOME payloadTy }) = P.Fragment "exception " :: P.Fragment (TypedSyntax.print_VId tagName) :: P.Fragment " of " :: doTy 0 payloadTy
