...
sig
  ...
  val applyAnnotation: Message.handler
                       -> string * LanguageOptions.options
                       -> LanguageOptions.options
  val doDecs: Context
              -> LanguageOptions.options
              -> Env
              -> MLBSyntax.BasDec list
              -> Code
              -> Env * Code
  val doMlbSource: Context -> Env -> string -> Code -> Env * Code
  datatype path_setting = PATH_MAP of string | PATH_VAR of string
  val loadPathVar: Message.handler
                   -> path_setting * string StringMap.map
                   -> string StringMap.map
end =
struct
  local structure M = MLBSyntax
  in
    type Context =
      { driverContext: Driver.Context
      , baseDir: string
    ...
    withtype Env =
      { bas: Env' M.BasMap.map
      , fixity: Fixity.Env (* fixity & id status *)
      , typing: Typing.Env
      }
    val emptyEnv: Env =
      {bas = M.BasMap.empty, fixity = Fixity.emptyEnv, typing = Typing.emptyEnv}
    ...
    fun applyAnnotation messageHandler (ann, langopt) =
      let
        fun unrecognized () =
          ( Message.warning
              (messageHandler, [], "MLB", "unrecognized annotation: " ^ ann)
          ; langopt
          )
      in
        case String.tokens Char.isSpace ann of
          ["nonexhaustiveBind", x] =>
            (case parseIgnoreWarnError x of
               SOME x => LanguageOptions.set.nonexhaustiveBind x langopt
             | NONE => unrecognized ())
        | ["nonexhaustiveMatch", x] =>
            (case parseIgnoreWarnError x of
               SOME x => LanguageOptions.set.nonexhaustiveMatch x langopt
             | NONE => unrecognized ())
        | ["nonexhaustiveRaise", x] =>
            (case parseIgnoreWarnError x of
        ...
        | ["valDescInComments", x] =>
            (case parseIgnoreWarnError x of
             ...
             | NONE => unrecognized ())
        | [name, value] =>
            (case LanguageOptions.setByName name of
               SOME setter =>
                 (case value of
    ...
    fun doDec (ctx: Context) langopt env (M.BasisDec binds) acc =
          let
            val (bas, acc) =
              List.foldl
                (fn ((basid, basexp), (bas, acc)) =>
                   let val (env', acc) = doExp ctx langopt env basexp acc
                   in (M.BasMap.insert (bas, basid, MkEnv env'), acc)
                   end) (M.BasMap.empty, acc) binds
          in
            (envWithBasis bas, acc)
          end
      | doDec _ _ env (M.OpenDec basids) acc =
          let
            val env' =
              List.foldl
                (fn (basid, newenv) =>
                   ...
                   | NONE => raise Fail ("undefined basis: " ^ basid)) emptyEnv
                basids
          in
            (env', acc)
          end
      | doDec ctx langopt env (M.LocalDec (decs1, decs2)) acc =
          let
            val (env', acc) = doDecs ctx langopt env decs1 acc
            val (env'', acc) =
              doDecs ctx langopt (mergeEnv (env, env')) decs2 acc
            val typingEnv =
          ...
          in
            ( {bas = #bas env'', fixity = #fixity env'', typing = typingEnv}
            , acc
            )
          end
      | doDec _ _ env (M.StructureDec binds) acc =
          let
            val strMap_fixity = #strMap (#idStatusMap (#fixity env))
            val strMap_fixity' =
              List.foldl
                (fn ((id1, id2), m) =>
                   ...
                   | NONE =>
                       raise Fail
                         ("undefined structure: " ^ Syntax.print_StrId id2))
          ...
          in
            ( { bas = M.BasMap.empty
              , fixity = Fixity.envWithStrMap strMap_fixity'
              , typing = Typing.envWithStrMap strMap_typing'
              }
            , acc
            )
          end
      | doDec _ _ env (M.SignatureDec binds) acc =
          let
            val sigMap_fixity = #sigMap (#fixity env)
            val sigMap_fixity' =
              List.foldl
                (fn ((id1, id2), m) =>
                   ...
                   | NONE =>
                       raise Fail
                         ("undefined signature: " ^ Syntax.print_SigId id2))
          ...
          in
            ( { bas = M.BasMap.empty
              , fixity = Fixity.envWithSigMap sigMap_fixity'
              , typing = Typing.envWithSigMap sigMap_typing'
              }
            , acc
            )
          end
      | doDec _ _ env (M.FunctorDec binds) acc =
          let
            val funMap_fixity = #funMap (#fixity env)
            val funMap_fixity' =
              List.foldl
                (fn ((id1, id2), m) =>
                   ...
                   | NONE =>
                       raise Fail
                         ("undefined functor: " ^ Syntax.print_FunId id2))
          ...
          in
            ( { bas = M.BasMap.empty
              , fixity = Fixity.envWithFunMap funMap_fixity'
              , typing = Typing.envWithFunMap funMap_typing'
              }
            , acc
            )
          end
      | doDec ctx langopt env (M.PathDec path) acc =
      ...
      | doDec ctx langopt env (M.AnnotationDec (anns, decs)) acc =
          let
            val langopt =
              List.foldl (applyAnnotation (#messageHandler ctx)) langopt anns
          in
            doDecs ctx langopt env decs acc
          end
      | doDec _ _ _ M.PrimDec acc =
          ( { bas = M.BasMap.empty
            , fixity = InitialEnv.initialFixityEnv
            , typing = InitialEnv.initialEnv
            }
          , acc
          )
      | doDec _ _ _ M.PrimOverloadDec acc =
          ( { bas = M.BasMap.empty
            , fixity = Fixity.emptyEnv
            , typing = InitialEnv.primOverloadEnv
            }
          , acc
          )
    and doDecs ctx langopt env decs acc =
      List.foldl
        (fn (dec, (newenv, acc)) =>
           let
             val (env', acc) =
               doDec ctx langopt (mergeEnv (env, newenv)) dec acc
           in
             (mergeEnv (newenv, env'), acc)
           end) (emptyEnv, acc) decs
    and doExp ctx langopt env (M.BasisExp decs) acc =
          doDecs ctx langopt env decs acc
      | doExp _ _ env (M.BasIdExp basid) acc =
          (case M.BasMap.find (#bas env, basid) of
             SOME (MkEnv env) => (env, acc)
           | NONE => raise Fail ("undefined basis: " ^ basid))
      | doExp ctx langopt env (M.LetExp (decs, exp)) acc =
          let val (env', acc) = doDecs ctx langopt env decs acc
          in doExp ctx langopt (mergeEnv (env, env')) exp acc
          end
    and doSmlSource ctx langopt env path acc =
      let
        val path = OS.Path.mkAbsolute
          {path = M.evalPath (#pathMap ctx) path, relativeTo = #baseDir ctx}
      ...
      in
        ...
        , { tynameset = tynameset
          , toFEnv = toFEnv
          , fdecs = #fdecs acc @ fdecs
          , cache = #cache acc
          }
        )
      end
    and doMlbSource ctx _ path acc =
      let
        val baseDir = #baseDir ctx
        val path = OS.Path.mkAbsolute
          {path = M.evalPath (#pathMap ctx) path, relativeTo = baseDir}
      in
        case StringMap.find (#cache acc, path) of
          NONE =>
            let
              val content = let val ins = TextIO.openIn path (* may raise Io *)
                            in TextIO.inputAll ins before TextIO.closeIn ins
                            end
            in
              case
                MLBParser.P.runParser MLBParser.basfile MLBParser.initialState
                  path
                  (StringStream.fromString {file = path, content = content})
              of
                MLBParser.P.Ok (decs, _) =>
                  let
                    val ctx' =
                      { driverContext = #driverContext ctx
    ...
    fun loadPathVar messageHandler (PATH_MAP file, pathMap) =
              ...
              (case TextIO.inputLine ins of
               ...
               | SOME line =>
                   (case String.tokens Char.isSpace line of
                      [name, value] =>
                        let
                          val path = M.evalPath pathMap value
                    ...
                    | unrecognized =>
                        let
                          val pos = {file = file, line = n, column = 1}
                          ...
                          case unrecognized of
                            _ :: _ :: _ =>
                              Message.error
                                ...
                                , [span]
                                , "MLB path map"
                                , "invalid line: path must not contain spaces"
                                )
                          | _ =>
                              Message.error
                                ( messageHandler
          ...
          in
            loop (TextIO.openIn file, 1, pathMap)
          end
      | loadPathVar messageHandler (PATH_VAR setting, pathMap) =
          (case String.tokens (fn c => c = #"=") setting of
             [name, value] =>
               StringMap.insert (pathMap, name, M.evalPath pathMap value)
           | _ =>
               ( Message.error
                   ( messageHandler
                   , []
                   , "MLB path map"
                   , "invalid --mlb-path-var option"
                   )
               ; pathMap
               ))
  end (* local *)
