...
fun toStringPrec (prec, ANY) = "_"
  | toStringPrec (prec, SOME_INT) = "<some integer>"
  | toStringPrec (prec, SOME_WORD) = "<some word>"
  | toStringPrec (prec, CHAR c) = "#\"" ^ Char.toString c ^ "\""
  | toStringPrec (prec, CHAR16 i) = if i <= 127 then
  ...
  | toStringPrec (prec, SOME_STRING) = "<some string>"
  | toStringPrec (prec, SOME_STRING16) = "<some string>"
  | toStringPrec (prec, RECORD fields) = let fun doTuple (i, [], acc) = SOME (List.rev acc)
  ...
  | toStringPrec (prec, VALCON (vid, NONE)) = Syntax.getVIdName vid
  | toStringPrec (prec, VALCON (vid, SOME payload)) = if prec >= 1 then
                                                          "(" ^ Syntax.getVIdName vid ^ " " ^ toStringPrec (1, payload) ^ ")"
                                                      else
                                                          Syntax.getVIdName vid ^ " " ^ toStringPrec (1, payload)
  | toStringPrec (prec, SOME_EXNCON) = "<some exception>"
  | toStringPrec (prec, VECTOR elems) = "#[" ^ String.concatWith "," (List.map toString elems) ^ "]"
...
datatype set = VALCON of { seen : Syntax.VIdSet.set, all : Syntax.VIdSet.set, withPayload : Syntax.VIdSet.set }
             ...
             | EMPTY
fun addValCon (VALCON { seen, all, withPayload }, { tag, allConstructors, constructorsWithPayload, representation }, hasPayload)
  ...
  | addValCon (EMPTY, { tag, allConstructors, constructorsWithPayload, representation }, hasPayload)
    = let val tag = Syntax.MkVId tag
...
fun isComplete (VALCON { seen, all, withPayload }) = Syntax.VIdSet.equal (seen, all)
  ...
  | isComplete (RECORD labels) = true
  | isComplete (VECTOR (seen, NONE)) = false
...
fun collectConstructors (F.WildcardPat _, acc) = acc
  ...
  | collectConstructors (F.RecordPat { sourceSpan, fields, ellipsis, allFields }, acc) = ConstructorSet.addRecord (acc, allFields)
  | collectConstructors (F.ValConPat { sourceSpan, info, payload }, acc) = ConstructorSet.addValCon (acc, info, Option.isSome payload)
  | collectConstructors (F.ExnConPat { sourceSpan, tagPath, payload }, acc) = ConstructorSet.addExnCon acc
  | collectConstructors (F.LayeredPat (_, _, _, innerPat), acc) = collectConstructors (innerPat, acc)
  | collectConstructors (F.VectorPat (_, pats, ellipsis, ty), acc) = ConstructorSet.addVector (acc, Vector.length pats, ellipsis)
fun specializeValCon (con, hasPayload)
    = let fun goPat (F.WildcardPat span, ps) = if hasPayload then
            ...
            | goPat (F.ValConPat { sourceSpan, info = { tag, ... }, payload = NONE }, ps)
              = if Syntax.MkVId tag = con andalso not hasPayload then
                    [ps]
                else
                    []
            | goPat (F.ValConPat { sourceSpan, info = { tag, ... }, payload = SOME (_, pat) }, ps)
