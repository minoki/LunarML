...
fun toStringPrec (_, ANY) = "_"
  ...
  | toStringPrec (_, CHAR c) = "#\"" ^ Char.toString c ^ "\""
  | toStringPrec (_, CHAR16 i) = if i <= 127 then
                                     "#\"" ^ Char.toString (Char.chr i) ^ "\""
                                 else if i <= 0xff then
                                     "#\"\\u00" ^ Int.fmt StringCvt.HEX i ^ "\""
                                 else if i <= 0xfff then
                                     "#\"\\u0" ^ Int.fmt StringCvt.HEX i ^ "\""
                                 else
                                     "#\"\\u" ^ Int.fmt StringCvt.HEX i ^ "\""
  | toStringPrec (_, SOME_STRING) = "<some string>"
  | toStringPrec (_, SOME_STRING16) = "<some string>"
  | toStringPrec (_, RECORD fields) = let fun doTuple (_, [], acc) = SOME (List.rev acc)
                                            | doTuple (i, (Syntax.NumericLabel j, x) :: fields, acc) = if i = j then
                                                                                                           doTuple (i + 1, fields, x :: acc)
                                                                                                       else
                                                                                                           NONE
                                            | doTuple (_, _, _) = NONE
                                      in case doTuple (1, fields, []) of
                                             SOME elems => "(" ^ String.concatWith ", " (List.map toString elems) ^ ")"
                                           | NONE => "{" ^ String.concatWith ", " (List.map (fn (Syntax.IdentifierLabel name, x) => name ^ "=" ^ toString x
                                                                                            | (Syntax.NumericLabel i, x) => Int.toString i ^ "=" ^ toString x) fields) ^ "}"
                                      end
  | toStringPrec (_, VALCON (vid, NONE)) = Syntax.getVIdName vid
  | toStringPrec (prec, VALCON (vid, SOME payload)) = if prec >= 1 then
                                                          "(" ^ Syntax.getVIdName vid ^ " " ^ toStringPrec (1, payload) ^ ")"
                                                      else
                                                          Syntax.getVIdName vid ^ " " ^ toStringPrec (1, payload)
  | toStringPrec (_, SOME_EXNCON) = "<some exception>"
  | toStringPrec (_, VECTOR elems) = "#[" ^ String.concatWith "," (List.map toString elems) ^ "]"
...
structure ConstructorSet = struct
datatype set = VALCON of { seen : Syntax.VIdSet.set, all : Syntax.VIdSet.set, withPayload : Syntax.VIdSet.set }
             | EXNCON
             ...
             | EMPTY
fun addValCon (VALCON { seen, all, withPayload }, { tag, allConstructors, constructorsWithPayload = _, representation = _ }, hasPayload)
    = let val tag = Syntax.MkVId tag
      in VALCON { seen = Syntax.VIdSet.add (seen, tag)
                , all = Syntax.VIdSet.union (all, allConstructors)
                , withPayload = if hasPayload then Syntax.VIdSet.add (withPayload, tag) else withPayload
                }
      end
  | addValCon (EMPTY, { tag, allConstructors, constructorsWithPayload, representation = _ }, _)
    = let val tag = Syntax.MkVId tag
      in VALCON { seen = Syntax.VIdSet.singleton tag
                , all = allConstructors
                , withPayload = constructorsWithPayload
                }
      end
  | addValCon _ = raise Fail "invalid pattern: value constructor"
fun addSCon (INT seen, F.IntegerConstant x) = INT (IntInfSet.add (seen, x))
  | addSCon (EMPTY, F.IntegerConstant x) = INT (IntInfSet.singleton x)
  | addSCon (WORD seen, F.WordConstant x) = WORD (IntInfSet.add (seen, x))
  | addSCon (EMPTY, F.WordConstant x) = WORD (IntInfSet.singleton x)
  | addSCon (CHAR seen, F.CharConstant x) = CHAR (CharSet.add (seen, x))
  | addSCon (EMPTY, F.CharConstant x) = CHAR (CharSet.singleton x)
  | addSCon (CHAR16 seen, F.Char16Constant x) = CHAR16 (IntRedBlackSet.add (seen, x))
  | addSCon (EMPTY, F.Char16Constant x) = CHAR16 (IntRedBlackSet.singleton x)
...
fun addRecord (set as RECORD fields, fields') = if Syntax.LabelSet.equal (fields, fields') then
                                                    set
                                                else
                                                    raise Fail "invalid pattern: record field"
  | addRecord (EMPTY, fields) = RECORD fields
  | addRecord _ = raise Fail "invalid pattern: record"
fun addVector (VECTOR (seen, allAbove), n, false) = VECTOR (IntRedBlackSet.add (seen, n), allAbove)
  | addVector (VECTOR (seen, NONE), n, true) = VECTOR (seen, SOME n)
  | addVector (VECTOR (seen, SOME m), n, true) = VECTOR (seen, SOME (Int.min (m, n)))
  | addVector (EMPTY, n, false) = VECTOR (IntRedBlackSet.singleton n, NONE)
  | addVector (EMPTY, n, true) = VECTOR (IntRedBlackSet.empty, SOME n)
...
fun collectConstructors (F.WildcardPat _, acc) = acc
  | collectConstructors (F.SConPat { scon, ... }, acc) = ConstructorSet.addSCon (acc, scon)
  | collectConstructors (F.VarPat _, acc) = acc
  | collectConstructors (F.RecordPat { sourceSpan = _, fields = _, ellipsis = _, allFields }, acc) = ConstructorSet.addRecord (acc, allFields)
  | collectConstructors (F.ValConPat { sourceSpan = _, info, payload }, acc) = ConstructorSet.addValCon (acc, info, Option.isSome payload)
  | collectConstructors (F.ExnConPat { sourceSpan = _, tagPath = _, payload = _ }, acc) = ConstructorSet.addExnCon acc
  | collectConstructors (F.LayeredPat (_, _, _, innerPat), acc) = collectConstructors (innerPat, acc)
  | collectConstructors (F.VectorPat (_, pats, ellipsis, _), acc) = ConstructorSet.addVector (acc, Vector.length pats, ellipsis)
fun specializeValCon (con, hasPayload)
    = let fun goPat (F.WildcardPat span, ps) = if hasPayload then
            ...
            | goPat (F.ValConPat { sourceSpan = _, info = { tag, ... }, payload = NONE }, ps)
              = if Syntax.MkVId tag = con andalso not hasPayload then
                    [ps]
                else
                    []
            | goPat (F.ValConPat { sourceSpan = _, info = { tag, ... }, payload = SOME (_, pat) }, ps)
              = if Syntax.MkVId tag = con andalso hasPayload then
                    [pat :: ps]
                else
                    []
    ...
    = let fun goPat (F.WildcardPat span, ps) = if hasPayload then
            ...
            | goPat (F.ExnConPat { sourceSpan = _, tagPath, payload = NONE }, ps)
              = if sameExp (tag, tagPath) andalso not hasPayload then
                    ...
                    []
            | goPat (F.ExnConPat { sourceSpan = _, tagPath, payload = SOME (_, pat) }, ps)
...
fun specializeSCon scon
    = let fun goPat (F.WildcardPat _, ps) = [ps]
...
and getFieldFromPat (_, pat as F.WildcardPat _) = pat
  ...
  | getFieldFromPat (label, F.RecordPat { sourceSpan = _, fields, ellipsis, allFields = _ }) = getFieldFromList (label, fields, ellipsis)
...
fun specializeRecord labels
    = let fun goPat (F.WildcardPat span, ps) = [List.tabulate (Syntax.LabelSet.numItems labels, fn _ => F.WildcardPat span) @ ps]
            ...
            | goPat (F.RecordPat { sourceSpan = _, fields, ellipsis, allFields }, ps) = if Syntax.LabelSet.equal (labels, allFields) then
                                                                                            [Syntax.LabelSet.foldr (fn (label, ps) => getFieldFromList (label, fields, ellipsis) :: ps) ps labels]
                                                                                        else
                                                                                            []
            | goPat (F.ValConPat _, _) = [] (* should not occur *)
            | goPat (F.ExnConPat _, _) = [] (* should not occur *)
...
fun specializeVector n
    = let fun goPat (F.WildcardPat span, ps) = [List.tabulate (n, fn _ => F.WildcardPat span) @ ps]
            | goPat (F.SConPat _, _) = [] (* should not occur *)
            | goPat (F.VarPat (span, _, _), ps) = [List.tabulate (n, fn _ => F.WildcardPat span) @ ps]
            | goPat (F.RecordPat _, _) = [] (* should not occur *)
            | goPat (F.ValConPat _, _) = [] (* should not occur *)
            | goPat (F.ExnConPat _, _) = [] (* should not occur *)
            | goPat (F.LayeredPat (_, _, _, pat), ps) = goPat (pat, ps)
            | goPat (F.VectorPat (span, pats, ellipsis, _), ps)
              = let val m = Vector.length pats
                in if m = n then
                       [Vector.foldr (op ::) ps pats]
                   else if m < n andalso ellipsis then
                       [Vector.foldr (op ::) (List.tabulate (n - m, fn _ => F.WildcardPat span) @ ps) pats]
                   else
                       []
                end
          fun goMatrix [] = []
...
local structure C = ConstructorSet in
datatype completeness = COMPLETE of ((F.Pat list list -> F.Pat list list) * (* arity *) int * (Example.example list -> Example.example)) list
                      | INCOMPLETE of Example.example option
fun isComplete (C.VALCON { seen, all, withPayload })
    = (let val x = Syntax.VIdSet.minItem (Syntax.VIdSet.difference (all, seen))
           val payload = if Syntax.VIdSet.member (withPayload, x) then
                             SOME Example.ANY
                         else
                             NONE
       in INCOMPLETE (SOME (Example.VALCON (x, payload)))
       end handle List.Empty => COMPLETE (Syntax.VIdSet.foldr (fn (vid, xs) =>
                                                                  let val hasPayload = Syntax.VIdSet.member (withPayload, vid)
                                                                      val (arity, construct) = if hasPayload then
                                                                                                   (1, fn [x] => Example.VALCON (vid, SOME x) | _ => raise Fail "invalid payload")
                                                                                               else
                                                                                                   (0, fn [] => Example.VALCON (vid, NONE) | _ => raise Fail "invalid payload")
                                                                  in (specializeValCon (vid, hasPayload), arity, construct) :: xs
                                                                  end
                                                              ) [] all)
      )
  | isComplete C.EXNCON = INCOMPLETE (SOME Example.SOME_EXNCON)
  ...
  | isComplete (C.CHAR seen) = let fun loop i = if i >= 256 then
                                                    ...
                                                    in if CharSet.member (seen, c) then
                                                           ...
                                                           INCOMPLETE (SOME (Example.CHAR (chr i)))
                                                    end
                               in loop 0
                               end
  ...
  | isComplete C.STRING = INCOMPLETE (SOME Example.SOME_STRING)
  | isComplete C.STRING16 = INCOMPLETE (SOME Example.SOME_STRING16)
  | isComplete (C.RECORD labels) = let fun construct fields = Example.RECORD (ListPair.zipEq (Syntax.LabelSet.toList labels, fields))
                                   in COMPLETE [(specializeRecord labels, Syntax.LabelSet.numItems labels, construct)]
                                   end
  | isComplete (C.VECTOR (seen, NONE)) = let fun loop i = if IntRedBlackSet.member (seen, i) then
                                                              loop (i + 1)
                                                          else
                                                              INCOMPLETE (SOME (Example.VECTOR (List.tabulate (i, fn _ => Example.ANY))))
                                         in loop 0
                                         end
  | isComplete (C.VECTOR (seen, SOME n)) = let fun loop i = if i >= n then
                                                                COMPLETE (List.tabulate (i, fn j => (specializeVector j, j, Example.VECTOR)))
                                                            else if IntRedBlackSet.member (seen, i) then
                                                                loop (i + 1)
                                                            else
                                                                INCOMPLETE (SOME (Example.VECTOR (List.tabulate (i, fn _ => Example.ANY))))
...
local
    fun defaultMatrix' ([], acc) = List.rev acc
      ...
      | defaultMatrix' ((F.WildcardPat _ :: ps) :: matrix, acc) = defaultMatrix' (matrix, ps :: acc)
      | defaultMatrix' ((F.SConPat _ :: _) :: matrix, acc) = defaultMatrix' (matrix, acc) (* no row *)
      | defaultMatrix' ((F.VarPat _ :: ps) :: matrix, acc) = defaultMatrix' (matrix, ps :: acc)
      | defaultMatrix' ((F.RecordPat _ :: _) :: matrix, acc) = defaultMatrix' (matrix, acc) (* no row *)
      | defaultMatrix' ((F.ValConPat _ :: _) :: matrix, acc) = defaultMatrix' (matrix, acc) (* no row *)
      | defaultMatrix' ((F.ExnConPat _ :: _) :: matrix, acc) = defaultMatrix' (matrix, acc) (* no row *)
      | defaultMatrix' ((F.LayeredPat (_, _, _, innerPat) :: ps) :: matrix, acc) = defaultMatrix' ((innerPat :: ps) :: matrix, acc)
      | defaultMatrix' ((F.VectorPat _ :: _) :: matrix, acc) = defaultMatrix' (matrix, acc) (* no row *)
in
fun defaultMatrix matrix = defaultMatrix' (matrix, [])
end
fun nonMatching ([], n) : (Example.example list) option = SOME (List.tabulate (n, fn _ => Example.ANY))
  | nonMatching (_ :: _, 0) = NONE
  | nonMatching (matrix, n)
    = let val firsts = List.map List.hd matrix
             ...
             let fun go [] = NONE
                   | go ((specialize, arity, construct) :: ss)
                     = case nonMatching (specialize matrix, arity + n - 1) of
                           SOME rs => let val (rs0, rs1) = ListUtil.splitAt (rs, arity)
...
fun useful ([], _) = true
  ...
  | useful (matrix, q :: qs) = let fun wildcard () = let val firsts = List.map List.hd matrix
                                                         val constructors = List.foldl collectConstructors ConstructorSet.EMPTY firsts
                                                     in case isComplete constructors of
                                                            COMPLETE specializers => List.exists (fn (specialize, arity, _) => useful (specialize matrix, List.tabulate (arity, fn _ => F.WildcardPat SourcePos.nullSpan) @ qs)) specializers
                                   ...
                                   fun goPat (F.WildcardPat _) = wildcard ()
                                     ...
                                     | goPat (F.ValConPat { sourceSpan = _, info = { tag, ... }, payload = SOME (_, innerPat) }) = useful (specializeValCon (Syntax.MkVId tag, true) matrix, innerPat :: qs)
                                     | goPat (F.ExnConPat { sourceSpan = _, tagPath, payload = NONE }) = useful (specializeExnCon (tagPath, false) matrix, qs)
                                     | goPat (F.ExnConPat { sourceSpan = _, tagPath, payload = SOME (_, innerPat) }) = useful (specializeExnCon (tagPath, true) matrix, innerPat :: qs)
                                     | goPat (F.LayeredPat (_, _, _, pat)) = goPat pat
                                     | goPat (F.VectorPat (_, pats, _, _)) = useful (specializeVector (Vector.length pats) matrix, Vector.foldr (op ::) qs pats)
                               in goPat q
                               end
...
fun goExp (ctx, F.PrimExp (_, _, exps)) = List.app (fn x => goExp (ctx, x)) exps
  ...
  | goExp (ctx, F.HandleExp { body, exnName = _, handler }) = (goExp (ctx, body); goExp (ctx, handler))
  | goExp (ctx, F.IfThenElseExp (x, y, z)) = (goExp (ctx, x); goExp (ctx, y); goExp (ctx, z))
  | goExp (ctx, F.CaseExp { sourceSpan, subjectExp, subjectTy = _, matches, matchType, resultTy = _ })
  ...
  | goExp (ctx, F.ProjectionExp { label = _, record, fieldTypes = _ }) = goExp (ctx, record)
  | goExp (ctx, F.TyAbsExp (_, _, exp)) = goExp (ctx, exp)
  | goExp (ctx, F.TyAppExp (exp, _)) = goExp (ctx, exp)
  | goExp (ctx, F.PackExp { payloadTy = _, exp, packageTy = _ }) = goExp (ctx, exp)
