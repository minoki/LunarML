...
val initialEnv : Typing.Env
    = let open Typing
      ...
      in { valMap = List.foldl (Syntax.VIdMap.unionWith #2)
                               Syntax.VIdMap.empty
                               [mkTopValConMap ([("ref", VId_ref, TypeScheme ([(tyVarA, NONE)], tyA --> refOf tyA)) (* forall 'a. 'a -> 'a ref *)
                                                ], Syntax.REP_REF)
                               ,mkTopValConMap ([("true", VId_true, TypeScheme ([], primTy_bool))
                                                ,("false", VId_false, TypeScheme ([], primTy_bool))
                                                ], Syntax.REP_BOOL)
                               ,mkTopValConMap ([("nil", VId_nil, TypeScheme ([(tyVarA, NONE)], listOf tyA)) (* forall 'a. 'a list *)
                                                ,("::", VId_DCOLON, TypeScheme ([(tyVarA, NONE)], mkPairType (tyA, listOf tyA) --> listOf tyA)) (* forall 'a. 'a * 'a list -> 'a list *)
                                                ], Syntax.REP_LIST)
                               ...
                               ,List.foldl (fn ((name, vid, tysc), m) => Syntax.VIdMap.insert(m, Syntax.MkVId name, (tysc, Syntax.ValueVariable, TypedSyntax.MkShortVId vid)))
                                           ...
                                           ,("_Prim.Vector.fromList", VId_Vector_fromList, TypeScheme ([(tyVarA, NONE)], listOf tyA --> vectorOf tyA))
                                           ...
                                           ,("_Prim.Vector.tabulate", VId_Vector_tabulate, TypeScheme ([(tyVarA, NONE)], mkPairType (primTy_int, primTy_int --> tyA) --> vectorOf tyA))
                                           ,("_Prim.Vector.concat", VId_Vector_concat, TypeScheme ([(tyVarA, NONE)], listOf (vectorOf tyA) --> vectorOf tyA))
                                           ,("_Prim.Array.array", VId_Array_array, TypeScheme ([(tyVarA, NONE)], mkPairType (primTy_int, tyA) --> arrayOf tyA))
                                           ,("_Prim.Array.fromList", VId_Array_fromList, TypeScheme ([(tyVarA, NONE)], listOf tyA --> arrayOf tyA))
                                           ,("_Prim.Array.tabulate", VId_Array_tabulate, TypeScheme ([(tyVarA, NONE)], mkPairType (primTy_int, primTy_int --> tyA) --> arrayOf tyA))
         ...
         , tyConMap = List.foldl (fn ((name, tystr), m) => Syntax.TyConMap.insert(m, Syntax.MkTyCon name, tystr))
                                 ...
                                 ,("list", { typeFunction = TypeFunction ([tyVarA], listOf tyA)
                                           , valEnv = mkValConMap ([("nil", TypeScheme ([(tyVarA, NONE)], listOf tyA))
                                                                   ,("::", TypeScheme ([(tyVarA, NONE)], mkPairType (tyA, listOf tyA) --> listOf tyA))
                                                                   ], Syntax.REP_LIST)
                                           }
                                  )
                                 ,("ref", { typeFunction = TypeFunction ([tyVarA], refOf tyA)
                                          , valEnv = mkValConMap ([("ref", TypeScheme ([(tyVarA, NONE)], tyA --> refOf tyA))
                                                                  ], Syntax.REP_REF)
                                          }
                                  )
...
val primOverloadEnv : Typing.Env
    = let open Typing
      ...
      in { valMap = List.foldl (fn ((name, vid, tysc), m) => Syntax.VIdMap.insert (m, Syntax.MkVId name, (tysc, Syntax.ValueVariable, TypedSyntax.MkShortVId vid)))
                               Syntax.VIdMap.empty
                               [("abs", VId_abs,    TypeScheme ([(tyVarA, SOME TypedSyntax.IsSignedReal)], tyA --> tyA))                               (* forall 'a:realint. 'a -> 'a,        default: int -> int *)
                               ,("~",   VId_TILDE,  TypeScheme ([(tyVarA, SOME TypedSyntax.IsRing)],       tyA --> tyA))                               (* forall 'a:num.     'a -> 'a,        default: int -> int *)
                               ,("div", VId_div,    TypeScheme ([(tyVarA, SOME TypedSyntax.IsIntegral)],   mkPairType (tyA, tyA) --> tyA))             (* forall 'a:wordint. 'a * 'a -> 'a,   default: int * int -> int *)
                               ,("mod", VId_mod,    TypeScheme ([(tyVarA, SOME TypedSyntax.IsIntegral)],   mkPairType (tyA, tyA) --> tyA))             (* forall 'a:wordint. 'a * 'a -> 'a,   default: int * int -> int *)
                               ,("*",   VId_TIMES,  TypeScheme ([(tyVarA, SOME TypedSyntax.IsRing)],       mkPairType (tyA, tyA) --> tyA))             (* forall 'a:num.     'a * 'a -> 'a,   default: int * int -> int *)
                               ,("/",   VId_DIVIDE, TypeScheme ([(tyVarA, SOME TypedSyntax.IsReal)],       mkPairType (tyA, tyA) --> tyA))             (* forall 'a:Real.    'a * 'a -> 'a,   default: real * real -> real *)
                               ,("+",   VId_PLUS,   TypeScheme ([(tyVarA, SOME TypedSyntax.IsRing)],       mkPairType (tyA, tyA) --> tyA))             (* forall 'a:num.     'a * 'a -> 'a,   default: int * int -> int *)
                               ,("-",   VId_MINUS,  TypeScheme ([(tyVarA, SOME TypedSyntax.IsRing)],       mkPairType (tyA, tyA) --> tyA))             (* forall 'a:num.     'a * 'a -> 'a,   default: int * int -> int *)
                               ,("<",   VId_LT,     TypeScheme ([(tyVarA, SOME TypedSyntax.IsOrdered)],    mkPairType (tyA, tyA) --> primTy_bool))     (* forall 'a:numtxt.  'a * 'a -> bool, default: int * int -> bool *)
                               ,(">",   VId_GT,     TypeScheme ([(tyVarA, SOME TypedSyntax.IsOrdered)],    mkPairType (tyA, tyA) --> primTy_bool))     (* forall 'a:numtxt.  'a * 'a -> bool, default: int * int -> bool *)
                               ,("<=",  VId_LE,     TypeScheme ([(tyVarA, SOME TypedSyntax.IsOrdered)],    mkPairType (tyA, tyA) --> primTy_bool))     (* forall 'a:numtxt.  'a * 'a -> bool, default: int * int -> bool *)
                               ,(">=",  VId_GE,     TypeScheme ([(tyVarA, SOME TypedSyntax.IsOrdered)],    mkPairType (tyA, tyA) --> primTy_bool))     (* forall 'a:numtxt.  'a * 'a -> bool, default: int * int -> bool *)
                               ]
