...
struct
  ...
  type counter = {counter: int ref, errorTolerance: int}
  type handler = {counter: counter, handler: message -> unit}
  exception Abort
  fun newCounter {errorTolerance} : counter =
    {counter = ref 0, errorTolerance = errorTolerance}
  fun newHandler (counter, handler) : handler =
  ...
  fun error
    ( {counter = {counter, errorTolerance}, handler}: handler
    , spans
    , domain
    , message
    ) =
    let
      val () = handler
        {spans = spans, domain = domain, message = message, type_ = ERROR}
      val numberOfErrors = !counter + 1
    in
      counter := numberOfErrors;
      if numberOfErrors > errorTolerance then raise Abort else ()
    end
