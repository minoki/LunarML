...
open String32
fun str (c : Char32.char) : String32.string
  = let val i = Char32.ord c
        (* big endian *)
        val a3 = Int.div (i, 0x1000000)
        val b3 = Int.mod (i, 0x1000000)
        val a2 = Int.div (b3, 0x10000)
        val b2 = Int.mod (b3, 0x10000)
        val a1 = Int.div (b2, 0x100)
        val a0 = Int.mod (b2, 0x100)
    in Lua.unsafeFromValue (Lua.call1 Lua.Lib.string.char #[Lua.fromInt a3, Lua.fromInt a2, Lua.fromInt a1, Lua.fromInt a0])
    end
              ...
              SOME (#"\\", strm') => (case getc strm' of
                                        ...
                                        | SOME (c, strm'') => if isOctDigit c then
                                                                  case scanOctalDigits (getc, strm'', 2, digitToInt c) of
                                                                      (value, strm''') => Parsed (Char32.chr value, strm''')
                                                              else
                                                                  Error
                                        | NONE => Error
                                     )
...
fun sub (s : String32.string, i : int) : Char32.char
  = let val op + = Lua.+
        val op * = Lua.*
        val ii = Lua.fromInt 4 * Lua.fromInt i
        val (a3, a2, a1, a0) = Lua.call4 Lua.Lib.string.byte #[Lua.unsafeToValue s, ii + Lua.fromInt 1, ii + Lua.fromInt 4]
    in Lua.unsafeFromValue (Lua.fromInt 0x1000000 * a3 + Lua.fromInt 0x10000 * a2 + Lua.fromInt 256 * a1 + a0) (* big endian *)
    end
end;
...
fun contains (s : String32.string) (c : Char32.char) : bool
    = let fun go i = if i >= String32.size s then
                         false
                     else if UnsafeString32.sub (s, i) = c then
                         true
                     else
                         go (i + 1)
      in go 0
                     ...
                     case c of
                       ...
                       | _ => let val x = Char32.ord c
                              in if x < 32 then
                                     ...
                                     Lua.unsafeFromValue (Lua.call1 Lua.Lib.string.format #[Lua.fromString "\\U%08X", Lua.fromInt x])
                              end
fun toCString #"\\" = "\\\\"
  | toCString #"\"" = "\\\""
  | toCString #"?" = "\\?"
...
fun substring (s : String32.string, i : int, j : int) : String32.string
    = if i < 0 orelse j < 0 orelse String32.size s < i + j then
          ...
          let val result = Lua.call1 Lua.Lib.string.sub #[Lua.unsafeToValue s, Lua.fromInt (4 * i + 1), Lua.fromInt (4 * (i + j))]
...
fun extract (s : String32.string, i : int, NONE : int option) : String32.string
    = if i < 0 orelse String32.size s < i then
          ...
          let val result = Lua.call1 Lua.Lib.string.sub #[Lua.unsafeToValue s, Lua.fromInt (4 * i + 1)]
          in Lua.unsafeFromValue result
          end
  | extract (s, i, SOME j) = substring (s, i, j)
fun concat (l : String32.string list) : String32.string
    = let val result = Lua.call1 Lua.Lib.table.concat #[Lua.unsafeToValue (Vector.fromList l)]
      in Lua.unsafeFromValue result
      end
fun concatWith (s : String32.string) (l : String32.string list) : String32.string
    = let val result = Lua.call1 Lua.Lib.table.concat #[Lua.unsafeToValue (Vector.fromList l), Lua.unsafeToValue s]
      in Lua.unsafeFromValue result
      end
fun implodeRev (l : Char32.char list) : String32.string
    = let val v = _primCall "Vector.unsafeFromListRevN" (List.length l, l)
          val result = Lua.call1 Lua.Lib.table.concat #[Lua.unsafeToValue (Vector.map String32.str v)]
      in Lua.unsafeFromValue result
      end
fun implode (l : Char32.char list) : String32.string
    = let val result = Lua.call1 Lua.Lib.table.concat #[Lua.unsafeToValue (Vector.map String32.str (Vector.fromList l))]
      in Lua.unsafeFromValue result
      end
...
fun isSubstring needle haystack : bool
  = let val m = String32.size needle
        val n = String32.size haystack - m
        fun go i = if i > n then
                       false
...
local
    ...
    type array = Char32.char Array.array
...
fun explode { base, start, length } = let fun loop (i, acc) = if i < start then
                                                                  ...
                                                                  loop (i - 1, String32.sub (base, i) :: acc)
...
structure Char32Vector = Text32Impl.UnsafeCharVector
structure Char32Array = Text32Impl.UnsafeCharArray
open Unsafe
end;
