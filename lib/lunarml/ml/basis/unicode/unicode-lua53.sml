structure Unicode :> UNICODE where type uchar = UChar.uchar = struct
  ...
  structure UTF8 = struct
    ...
    fun encode1 x = _primCall "UTF8.str" (x)
    fun isWellFormed s = _primCall "UTF8.isWellFormed" (s) (* utf8.len on Lua 5.4 (utf8.len on Lua 5.3 allows surrogates) *)
    fun decode s =
      let fun go (acc, s) =
                case decode1 s of
      ...
      fun decode1 ss =
          ...
          in
            if n > 0 then
              ...
              let val i' = _primCall "UTF8.offset" (s, i)
              in SOME (_primCall "UTF8.codePointAt" (s, i), Substring.substring (s, i', n - i' + i))
              end
            else
              NONE
          end
  ...
  structure String = struct
    ...
    fun size s = _primCall "UTF8.size" (s) (* utf8.len *)
    fun foldl f init s = USubstring.foldl f init (Substring.full s)
    fun explode s = List.rev (foldl (op ::) [] s)
    fun implode xs = String.concat (List.map UTF8.encode1 xs) (* utf8.char? *)
    fun map f s = String.concat (List.rev (foldl (fn (c, acc) => UTF8.encode1 (f c) :: acc) [] s)) (* TODO: Use string.gsub? *)
    fun app f s = USubstring.app f (Substring.full s) (* utf8.codes? *)
    fun all f s = USubstring.all f (Substring.full s) (* utf8.codes? *)
    fun exists f s = USubstring.exists f (Substring.full s) (* utf8.codes? *)
