...
val sqrtBase64 : Int64.int = LunarML.assumeDiscardable (fn () => Int64.fromInt (Word.toInt sqrtBaseW)) ()
fun toInt ZERO = 0
...
fun toInt64 ZERO = 0
  | toInt64 (POSITIVE words) = Vector.foldr (fn (lo, hi) => hi * sqrtBase64 * sqrtBase64 + _primCall "Word.toInt64.unchecked" (lo)) 0 words (* may raise Overflow *)
  | toInt64 (NEGATIVE words) = Vector.foldr (fn (lo, hi) => hi * sqrtBase64 * sqrtBase64 - _primCall "Word.toInt64.unchecked" (lo)) 0 words (* may raise Overflow *)
...
fun fromInt64 0 = ZERO
  | fromInt64 x =
    let val sqrtBase : Int64.int = Int64.fromInt (Word.toInt sqrtBaseW)
    in if x > 0 then
           let fun go 0 = []
                 | go x = let val q1 = x div sqrtBase
                              val r1 = Int64.toInt (x mod sqrtBase)
                              val q2 = q1 div sqrtBase
                              val r2 = Int64.toInt (q1 mod sqrtBase)
                          in (Word.fromInt r1 + Word.fromInt r2 * sqrtBaseW) :: go q2
                          end
           in POSITIVE (Vector.fromList (go x))
           end
           ...
           let fun go 0 = []
                 | go x = let val q1 = x .Int64.quot. sqrtBase
                              val r1 = Int64.toInt (x .Int64.rem. sqrtBase)
                              val q2 = q1 .Int64.quot. sqrtBase
                              val r2 = Int64.toInt (q1 .Int64.rem. sqrtBase)
                          in (Word.fromInt (~ r1) + Word.fromInt (~ r2) * sqrtBaseW) :: go q2
                          end
           in NEGATIVE (Vector.fromList (go x))
           end
...
local
    fun wordToStringWidth (width, radix, x) =
        let val buf = _primCall "CharArray.alloc" (width)
            fun loop (i, x) = if i < 0 then
                                  ()
                              else
                                  let val d = x mod radix
                                      val c = if d < 0w10 then
                                                  Word.toInt d + Char.ord #"0"
                                              else
                                                  Word.toInt d + (Char.ord #"A" - 10)
                                  in _primCall "Unsafe.CharArray.update" (buf, i, Char.chr c)
                                   ; loop (i - 1, x div radix)
                                  end
        in loop (width - 1, x)
...
fun toStringAbs words = if Vector.length words = 0 then
                            ...
                            Word.fmt StringCvt.DEC (Vector.sub (words, 0))
                        else
                            let val (q, r) = quotRemAbsSingle (words, ten_to_9)
                            in toStringAbs q ^ wordToStringWidth (9, 0w10, r)
                            end
...
fun toOctStringAbs words = if Vector.length words = 0 then
                               ...
                               Word.fmt StringCvt.OCT (Vector.sub (words, 0))
                           else
                               let val (q, _) = RShiftAbs (words, 0w30)
                                   val r = Word.andb (Vector.sub (words, 0), 0wx3fffffff)
                               in toOctStringAbs q ^ wordToStringWidth (10, 0w8, r)
                               end
...
fun toHexStringAbs words = let val n = Vector.length words
                               ...
                               val lasts = Word.fmt StringCvt.HEX last
                               fun loop (xs, i) = if i >= n - 1 then
                                                      lasts :: xs
                                                  else
                                                      loop (wordToStringWidth (Word.wordSize div 4, 0w16, Vector.sub (words, i)) :: xs, i + 1)
