...
fun scanSubstring (getc, strm, s) = case Substring.getc s of
                                      ...
                                      | SOME (c, s') => case getc strm of
                                                            SOME (c', strm') => if c = c' orelse (Char.isAlpha c andalso Char.isAlpha c' andalso Char.toUpper c = Char.toUpper c') then
                                                                                    ...
                                                                                    NONE
                                                          | NONE => NONE
fun scanZeroOrMoreDigits (getc, strm, revAcc, n) = case getc strm of
                                                       SOME (c, strm') => if Char.isDigit c then
                                                                              scanZeroOrMoreDigits (getc, strm', c :: revAcc, n + 1)
                                                                          else
                                                                              (revAcc, n, strm)
                                                     | NONE => (revAcc, n, strm)
fun scanOneOrMoreDigits (getc, strm, revAcc) = case getc strm of
                                                   SOME (c, strm') => if Char.isDigit c then
                                                                          SOME (scanZeroOrMoreDigits (getc, strm', c :: revAcc, 1))
                                                                      else
                                                                          NONE
                                                 | NONE => NONE
fun scanOptFracPart (getc, strm, revAcc) = case getc strm of
                                               SOME (#".", strm') => (case scanOneOrMoreDigits (getc, strm', revAcc) of
                                                                          SOME (revAcc, n, strm'') => (revAcc, n, strm'')
                                                                        | NONE => (revAcc, 0, strm)
                                                                     )
                                             | _ => (revAcc, 0, strm)
fun scanOptExpPart (getc, strm) = case getc strm of
                                      SOME (c, strm') => if c = #"e" orelse c = #"E" then
                                                             ...
                                                             (0, strm)
                                    | NONE => (0, strm)
fun makeReal (isNegative, digits, exp) =
    let val digits' = case IntInf.scan StringCvt.DEC List.getItem digits of
                          SOME (digits', _) => digits'
                        | NONE => raise Fail "Real.scan: internal error"
        fun raise_ (x, n) = if n <= 0 then
                                x
                            else
                                raise_ (x * 10, n - 1)
        val (num, den) = if exp >= 0 then
                             (raise_ (digits', exp), 1)
                         else
                             (digits', raise_ (1, ~exp)) (* TODO: exponent overflow? *)
        val log2Num = IntInf.log2 num
        val log2Den = IntInf.log2 den
        (* *)
    in raise Fail "not implemented yet"
    end
(* [+~-]? ([0-9]+(.[0-9]+?)?|.[0-9]+)([eE][+~-]?[0-9]+?)? / [+~-]?(inf|infinity|nan) *)
fun scan getc strm = let val strm = skipInitialWhitespace (getc, strm)
                         val (isNegative, strm) = scanSign (getc, strm)
                     in case getc strm of
                            SOME (#".", strm') => (case scanOneOrMoreDigits (getc, strm', []) of
                                                       SOME (revAcc, nFrac, strm'') =>
                                                           let val (e, strm''') = scanOptExpPart (getc, strm'')
                                                           in SOME (makeReal (isNegative, List.rev revAcc, e - nFrac), strm''')
                                                           end
                                                     | NONE => NONE
                                                  )
                          | SOME (c, strm') => if c = #"i" orelse c = #"I" then
                                                   ...
                                                   let val (revAcc, _, strm'') = scanZeroOrMoreDigits (getc, strm', [c], 0)
                                                       val (revAcc, nFrac, strm''') = scanOptFracPart (getc, strm'', revAcc)
                                                       val (e, strm'''') = scanOptExpPart (getc, strm''')
                                                   in SOME (makeReal (isNegative, List.rev revAcc, e - nFrac), strm'''')
                                                   end
                                               else
                                                   NONE
