...
fun checkOrdering (x : Word16.word, y : Word16.word)
    = let val result = (x < y, x <= y, x > y, x >= y)
          val list = List.map (fn f => f (x, y)) [Word16.<, Word16.<=, Word16.>, Word16.>=]
      in if x = y then
...
fun checkDivMod (x : Word16.word, y : Word16.word)
    = if y = 0w0 then
          ((Word16.div (x, y); false) handle Div => true)
          andalso ((Word16.mod (x, y); false) handle Div => true)
      else
          let val q = x div y
              val r = x mod y
              val q' = Word16.div $ (x, y)
              val r' = Word16.mod $ (x, y)
...
fun checkSmall (x, y)
    = if x > 0w200 then
          ...
          ( print ("Error with (x, y) = (0wx" ^ Word16.toString x ^ ", 0wx" ^ Word16.toString y ^ ")\n")
          ; false
          );
...
fun loop n = if n <= 0 then
                 ...
                 let val x = Word16.fromLarge (Word64.toLarge (gen ()))
                     val y = Word16.fromLarge (Word64.toLarge (gen ()))
