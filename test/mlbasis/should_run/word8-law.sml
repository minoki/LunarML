...
fun checkOrdering (x : Word8.word, y : Word8.word)
    = let val result = (x < y, x <= y, x > y, x >= y)
          val list = List.map (fn f => f (x, y)) [Word8.<, Word8.<=, Word8.>, Word8.>=]
      in if x = y then
...
fun checkDivMod (x : Word8.word, y : Word8.word)
    = if y = 0w0 then
          ((Word8.div (x, y); false) handle Div => true)
          andalso ((Word8.mod (x, y); false) handle Div => true)
      else
          let val q = x div y
              val r = x mod y
              val q' = Word8.div $ (x, y)
...
fun checkSmall (x, y)
    = if x > 0w200 then
          ...
          ( print ("Error with (x, y) = (0wx" ^ Word8.toString x ^ ", 0wx" ^ Word8.toString y ^ ")\n")
          ; false
          );
...
fun loop n = if n <= 0 then
                 ...
                 let val x = Word8.fromLarge (Word64.toLarge (gen ()))
                     val y = Word8.fromLarge (Word64.toLarge (gen ()))
